...
## Active Round: 2026-04-29-R310

Restart note: R248 closed the Windows server-data import gate with local `Crystal/Build/Server/Debug/Server.MirDB` plus matching `Build/Server/Debug/Envir/Routes`. R298/R300 remain the accepted stable-diff packet parity decision for the tracked backend/server slice. R301 refreshed the final automated Candidate acceptance pack. R302 proved original Crystal server/client visual-reference capture. R303 proved all 463 Crystal map files are source-present/parseable for sampled frontend loading. R304 fixed aligned Bichon current-map NPC population. R305 fixed first-pass aligned Bichon visible respawn population. R306 removed the default quest tracker overlay and normalized visible NPC/monster nameplates for the same Bichon comparison view. R307 added explicit ordinary Guard/ArcherGuard evidence for `0:287,618`. R308 removed the original-size browser stage downscale/frame decoration and exported missing visible-object sprite libs for that Bichon comparison. R309 closed the measured desktop minimap 2px overflow. R310 fixed the Web login transition leak into game screenshots, scoped over-broad NPC quest markers, and added original/Web visual-watch tooling. Real full-project accepted 1:1 remains roughly 90.0% until human Crystal visual/feel acceptance or explicit accepted differences close.
...
| [x] | Add explicit parity truth audit | Coordinator | `docs/PARITY-TRUTH-AUDIT.md`, handoff docs | Truth audit now separates Accepted, Candidate, Fallback, Blocked, and Product evolution. It explicitly calls out stable-diff packet acceptance scope, synthetic map fallback, product-evolution Admin surfaces, local persistence, and human acceptance boundaries. |
...
## Product Evolution Round: 2026-04-28-R254-R256

| Status | Task | Owner | Files | Notes |
| --- | --- | --- | --- | --- |
| [x] | Add DB-backed operator token auth and Admin Web login | Coordinator | `apps/admin-api`, `apps/admin-web`, `infra/postgres/migrations/0001_core.sql`, docs | Added `ADMIN_OPERATOR_AUTH_BACKEND=postgres`, `GET /admin/auth/me`, `admin_operators.token_hash`, `last_authenticated_at_ms`, token-aware operator writes, Admin Web `/login`, login/logout server actions, cookie/env token forwarding, and top-bar resolved identity. Live smoke proved unauthenticated 401, token login, and real `/admin/read/operators`. |
| [x] | Enforce peer approval semantics for high-risk commands | Coordinator | `apps/admin-api`, `apps/admin-web`, docs | Command submission now requires an approved record that matches command id, command type, and requesting operator. Self-approval remains blocked by default; Admin Web hides approve/reject actions for the requesting operator and shows peer-required state. Live smoke used lead request + peer approval + lead grant-currency submission. |
| [x] | Add automatic Gateway zone runtime heartbeat | Coordinator | `apps/gateway`, `apps/admin-api`, docs | Gateway reads `ADMIN_API_BASE_URL`, `MIR2_GATEWAY_ADMIN_OPERATOR_TOKEN`, and `MIR2_GATEWAY_ZONE_*` env, then periodically posts `gateway_heartbeat` zone runtime records to `/admin/servers/zones`. Live smoke read `gateway-r254-live` back through `/admin/read/servers`. |

## Product Evolution Round: 2026-04-29-R257
