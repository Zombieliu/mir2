...
> Latest product-evolution sync: 2026-05-05 completed for the production architecture boundary slice. `apps/simulation/src/runtime.rs` has been split into `apps/simulation/src/runtime/`, `mir2-simulation` now exposes `WorldRuntime` / `WorldCommand` / `InProcessWorldRuntime`, and `mir2-gateway` now opens sessions through `ZoneRegistry` with `SessionRouter`, `ZoneRuntimeFactory`, and `ZoneRuntimeHandle`. The default in-process zone shares player presence plus per-map NPC/monster/drop snapshot layers across sessions, tombstones removed shared drops for both high-level and protocol pickup paths, and tombstones removed non-player entity ids to prevent stale private session snapshots from resurrecting shared map objects. Architecture completion is tracked separately from Crystal parity and is now **52%** in `docs/ARCHITECTURE-IMPLEMENTATION-STATUS.md`. Verification passed: `mir2-simulation` check, `mir2-gateway` check, gateway lib tests 64/64, packet-trace bin tests 16/16, focused routing tests 9/9, `cargo +1.89.0 fmt -p mir2-gateway`, and `git diff --check`. Remaining architecture work is promoting combat mutation, AI ticks, remote drop pickup inventory gain, NPC services, and AOI deltas into true shared zone authority.
