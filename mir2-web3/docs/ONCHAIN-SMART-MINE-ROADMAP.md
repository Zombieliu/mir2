...
> **A 道（2026-06-10）：✅ 客户端接线 + sim 矿脉渲染 DONE（代码 + headless 门禁），对抗审查 8 发现全修。**
> **Sim**：env 门控矿脉映射 `MIR2_ONCHAIN_MINE_NODES`（`mine_id:map:x:y:max_stones`，默认关→无幽灵矿脉；两种 runtime 都接，网关 CrystalWorld 路径覆盖）；grant 收 `mine_id`/`stones_left` → `OnchainMineLedger` → 发 `MineNodeState`（GainedItem 在前、矿脉在后 = 客户端结算信号；**零产出批也转发/重播**）；StartGame **和传图** 都播种；分档与 P0 `mine_stage` 1:1（恰半=满）。**Web**：`onchain-mine-config.ts`（纯配置，@mysten/sui 不进主包，SDK 动态 import）+ `onchain-mine-state.ts` 纯状态机 + 钱包会话保留 + `OnchainMinePanel` HUD + page.tsx 接线（挥击=朝矿脉 attackDirection+攒批 → 自动出批[失败后须手动重试，防钱包弹窗死循环] → 签名 → 结算对账[矿量按在途挥数设上界] → 60s 看门狗弃批[不回灌，nonce 已花]）；`NEXT_PUBLIC_ONCHAIN_MINE=1` 总开关。**Relayer 修复**：`ore_amount=0` 的 `mine_settled` 现在转发（原丢弃会卡死客户端在途批）。
> 门禁：sim 套件全绿（含 6 个新 onchain 测试）+ `cargo fmt --check` + gateway check + relayer 9/9 + `tsc --noEmit` 0 错 + frontend-logic 全绿（onchain-mine 20 组）。运行手册：`docs/ONCHAIN-M4-E2E-RUNBOOK.md`（全本地 3 进程 + testnet；无任何组件部署上服务器——那是 M7）。
> **待真机（唯一剩余）**：照 runbook 起本地 gateway+relayer+web，浏览器钱包实链端到端冒烟（tx digest→背包→分档→redeem 得金币）。**M4 出口的"端到端 + 三处幂等全生效"须在真机复核后才算闭合。**
