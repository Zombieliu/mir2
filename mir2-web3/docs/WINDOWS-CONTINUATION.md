...
- If continuing product evolution instead of parity closure, read `docs/POST-1TO1-EVOLUTION-PLAN.md` and `docs/TECH-MODERNIZATION-RFC.md` first. Database, cache, login UI, admin backend, global zone, and NPC script parser changes are allowed evolution areas, but the current Candidate baseline should remain a regression reference.
