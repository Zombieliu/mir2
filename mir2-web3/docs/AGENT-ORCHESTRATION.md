...
> Latest product-evolution sync: 2026-04-29-R257 completed and human-accepted locally. Full local live acceptance is green for the Admin operations stack: Postgres/Redis/NATS/Redpanda/ClickHouse, Gateway, Admin API, Admin Web, and Player Web. The smoke covered Admin Web login, Operators, Approvals, peer-approved GM grant, Servers heartbeat, Audit, and Timeline; the user confirmed the checked surfaces are usable. Admin event/timeline ClickHouse filters now apply after aggregation, GM mail/grant receipts persist in Postgres, and local `ai/*.png` reference artifacts are ignored.
