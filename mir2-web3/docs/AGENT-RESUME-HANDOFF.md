...
> Latest product-evolution sync: 2026-04-28-R254-R256 completed. Admin auth now has strict Postgres operator-token mode through `ADMIN_OPERATOR_AUTH_BACKEND=postgres` and `/admin/auth/me`; Admin Web has token login/logout and top-bar identity from the Rust API; high-risk command submission requires a matching peer-approved request; Gateway automatically writes `gateway_heartbeat` zone runtime records to `/admin/servers/zones`. Remaining Admin production gaps are external IdP/session auth, richer approval workflows, support tooling, rate limits, deployment hardening, and broader GM executors.

> Latest product-evolution sync: 2026-04-28-R251-R253 completed. Admin Servers now has Postgres-backed zone runtime telemetry through `admin_zone_runtime_records` and `POST /admin/servers/zones`; Admin Operators/RBAC now has `admin_operators`, `/admin/read/operators`, `/admin/operators`, and a new `/operators` page. Console-wide page smoke returned 200 for all Admin Web pages. Remaining Admin production gaps are external IdP/session auth, production approval policy, broader support workflows, and additional GM executors.

> Latest product-evolution sync: 2026-04-28-R250 completed. Admin Activities, Economy price feeds, and Risk trade graph now use Postgres-backed projections with write routes and Admin Web server-action forms. `/admin/read/activities`, `/admin/read/economy`, and `/admin/read/risk` read real `admin_activities`, `admin_market_price_feeds`, and `admin_trade_graph_edges` rows when `ADMIN_DATABASE_URL` is configured. Remaining Admin read-model gap is deeper zone process telemetry.

> Latest product-evolution sync: 2026-04-28-R249 completed. Gateway now exposes `GET /admin/sessions` from the same in-memory/Redis `GatewaySessionCache` used for kick routing. Admin API overlays that live presence data onto Admin Web read models: Dashboard online count, Players online status/runtime HP/gold/map, Player Detail runtime presence, and Servers zones-online source now come from Gateway session cache instead of placeholders. Remaining unwired Admin read domains are activity config, market prices, trade graph, and deeper zone process telemetry.

> Latest product-evolution sync: 2026-04-28-R248 completed. Admin Web now uses true Rust Admin API read models instead of `mock-data.ts` for Dashboard, Players, Player Detail, Economy, Activities, Servers, and Risk. `/admin/read/*` derives player/economy/hot-map/risk data from JSON account-store or explicit Postgres source mode; activity config, market prices, trade graph, deeper zone telemetry, and online-session totals were left as empty/unwired states for follow-up slices.
...
> Latest backend parity sync: 2026-04-28-R300 completed. Stable-diff packet acceptance is now explicit and enforceable. R298 provides the 9/9 stable-clean live Crystal matrix, R299 identifies strict exact dirtiness as Crystal dynamic state, and R300 wires `packet_trace` accepted parity mode with `MIR2_PACKET_TRACE_ACCEPT_STABLE_DIFF=1`. Current truth wording: automated parity evidence **100% Candidate**, backend/server tracked slice **100% Accepted under explicit stable-diff packet acceptance**, whole-project accepted Crystal 1:1 **roughly 90%** until human visual/feel acceptance closes. Evidence: `docs/PACKET-PARITY-ACCEPTANCE.md` and `docs/generated/packet-traces/r300-stable-acceptance.json`. Verification: packet-trace bin 15/15 and `cargo +1.89.0 fmt --check`.

> Latest frontend/player QA sync: 2026-04-28-R297 completed. Windows refreshed automated Candidate evidence with full client resources at `E:\mir2\Crystal\Build\Client\Debug`: web build/typecheck passed, map API smoke served 18/18 requests, minimap smoke had 0 failures with known 450/451 warning, WS load passed 64/64 ready with 0 errors, and Stage 5 UI smoke captured 88 screenshots with `criticalConsoleErrorCount=0`. This round also hardened concurrent account-store JSON writes, aligned WS load with Crystal-style empty accounts, exposed minimap indices through gateway `MapInformation`, and exported missing original scene NPC/Monster sprite libs. Verification passed without Docker: `mir2-simulation` 674/674, `mir2-gateway` 55/55 plus packet-trace bin 14/14, `mir2-admin-api` 22/22, `fmt --check`, `git diff --check`, and web `tsc --noEmit`. R300 later closes the backend/server packet gate; whole-project accepted Crystal 1:1 remains **roughly 90%** until human visual/feel acceptance closes.

> Latest backend parity sync: 2026-04-28-R298 completed. Windows live Crystal stable packet matrix evidence is available at `docs/generated/packet-traces/r298-live-matrix/latest-matrix.json` with 9/9 local OK, 9/9 Crystal OK, `crystalMissingCount=0`, `stableDiffCleanCount=9`, and `acceptedStableLiveComparisonCount=9`. Strict exact diff remains dirty (`diffDirtyCount=9`, `acceptedLiveComparisonCount=0`), which R300 now treats as diagnostic rather than blocking for the explicit stable-diff packet gate. Verification passed without Docker: `mir2-simulation` 674/674, `mir2-gateway` 55/55 plus packet-trace bin 14/14, `mir2-admin-api` 22/22, `fmt --check`, `git diff --check`, and web `tsc --noEmit`.

> Latest backend parity sync: 2026-04-28-R248 completed. Windows closed the previously blocked Crystal server-data import gate. `generate-crystal-respawn-manifest.mjs` read `E:\mir2\Crystal\Build\Server\Debug\Server.MirDB` and `E:\mir2\Crystal\Build\Server\Debug\Envir\Routes`, refreshed generated Crystal respawn/monster/item/NPC-info manifests, and real map records now carry `no_throw_item`, `no_drop_player`, and `no_drop_monster`. Verification passed: `mir2-game-data` 22/22, focused `mir2-simulation no_drop_monster_map_rule` 2/2, full `mir2-simulation` 670/670, and `mir2-gateway` 55/55 plus packet-trace bin tests 7/7. Do not start Docker for this parity path unless product-evolution infra verification is explicitly resumed.

> Latest product-evolution sync: 2026-04-28-R247 completed. Admin Web GM Tools now has a reliable system-mail submit path and explicit post-submit status loading. The form submits via a server action with pending UI, Admin API exposes `GET /admin/commands/:command_id/status`, and `/gm-tools?commandId=...` renders command status/result/trace/operator plus the matching `mail-{commandId}` outbox receipt. Browser smoke verified `Queue System Mail` producing `succeeded`, `gateway_live / 1`, mail id `4`, and Player Web Mail showing `Compensation Package` with `5000 Gold · Unclaimed`.

> Latest product-evolution sync: 2026-04-28-R246 completed. Admin-delivered mail/gold now becomes visible to already-online player sessions. Gateway refreshes externally delivered Stage 5 mail from the shared account store before snapshots and saves, and Playwright verified the Player Web Mail panel showing `GM Currency Grant` with `888 Gold · Unclaimed` after a live Admin API grant.

> Latest product-evolution sync: 2026-04-28-R245 completed. The local admin/backend system is up and browser-testable: Gateway `127.0.0.1:7110`, Admin API `127.0.0.1:7420`, Admin Web `http://127.0.0.1:3020`, and Player Web `http://127.0.0.1:3010`. The stack uses Docker Postgres/Redis/NATS/Redpanda/ClickHouse, explicit Postgres account-store source mode, Redis routing cache, local bearer auth, and ClickHouse event reads. Admin API now supports `ADMIN_OPERATOR_POLICY_PATH` policy-file auth and blocks approval requester self-approval by default; the local smoke server was started with `ADMIN_APPROVAL_ALLOW_SELF=true` for single-operator testing.

> Latest product-evolution sync: 2026-04-27-R238 completed. Admin command analytics now cover `admin.command.succeeded`, `admin.command.failed`, and `admin.command.denied`; ClickHouse consumes all three Redpanda topics through `clickhouse-admin-events-v2`, and `/admin/events` filter smoke passed for failed and denied outcomes.

> Latest product-evolution sync: 2026-04-27-R237 completed. Admin outbox delivery state now records per-publisher NATS/Redpanda results, `last_error`, and `dispatched_at_ms`; partial publisher failure keeps rows in retry/dead-letter flow instead of marking them dispatched. Admin API `/admin/events` supports `limit`, `commandId`, `eventType`, and `status`, returns degraded ClickHouse responses, and Admin Web Audit exposes those filters.

> Latest product-evolution sync: 2026-04-27-R236 completed. Admin outbox events now use stable envelopes, `dispatch-admin-outbox` publishes to Redpanda Pandaproxy when `ADMIN_OUTBOX_REDPANDA_URL` is set while preserving NATS, ClickHouse projects to `admin_events` / `admin_command_events`, Admin API exposes `/admin/events`, and Admin Web Audit shows the event stream. End-to-end smoke passed from real Admin API command to Postgres outbox, Redpanda, ClickHouse, and API readback.

> Latest product-evolution sync: 2026-04-27-R235 completed. Redpanda and ClickHouse are now part of the default local dev Compose event/analytics stack. Redpanda uses internal/external Kafka listeners, and ClickHouse initializes a Kafka-engine projection for `admin.command.succeeded` into `mir2_events.admin_command_events`. NATS remains the existing admin command/notification outbox dispatcher.

> Latest product-evolution sync: 2026-04-27-R234 completed. Admin API production-boundary hardening now includes optional `ADMIN_OPERATOR_TOKEN` bearer auth, high-risk command `approvalId` validation, item/gold grant executors routed through audited system-mail delivery, and admin outbox retry/dead-letter transitions. Verification: admin-api 11/11.

> Latest product-evolution sync: 2026-04-27-R233 completed. Postgres account-store source mode now tracks loaded `store_version` / `save_version` metadata, rejects stale source writers, and refreshes local source-version metadata after successful saves. Docker Postgres tests cover stale writer rejection and reload-save success.

> Latest product-evolution sync: 2026-04-27-R232 expanded. Gateway session cache now has optional Redis support via `MIR2_GATEWAY_REDIS_CACHE_URL` with TTL via `MIR2_GATEWAY_SESSION_CACHE_TTL_SECONDS`; default remains in-memory when Redis env is unset. Redis roundtrip/remove/expire focused tests passed.

> Latest product-evolution sync: 2026-04-27-R232 completed. The first gateway cache boundary now exists as a deterministic in-memory slice before a Redis adapter: `apps/simulation::ActiveSessionIdentity` exposes active account/character identity, `apps/gateway::GatewaySessionCache` records online session snapshots, and the web gateway refreshes after authoritative saves plus removes the cache record on disconnect. Verification passed for focused gateway cache tests 4/4 and `cargo +1.89.0 fmt --check`; Redis remains non-authoritative and still needs the external adapter/invalidation slice.
...
> Latest truth-audit sync: 2026-04-28. Read `docs/PARITY-TRUTH-AUDIT.md` before using any percent-complete wording. Current honest status is **100% Candidate** for automated evidence, **100% Accepted under explicit stable-diff packet acceptance** for the backend/server tracked slice, and **roughly 90%** for whole-project accepted Crystal 1:1. Do not call the whole project 100% Accepted until human visual/feel acceptance is closed or explicitly accepted.
...
> Previous sync: R225 completed. Mac-local Candidate regression was green: web `tsc --noEmit`, direct `next build`, Stage 5 UI smoke (88 screenshots, manifest summary counts), map API smoke 18/18, minimap asset smoke 0 failures with known 450/451 warning, WS load 64/64, `mir2-game-data` 22/22, `mir2-gateway` 54/54, `mir2-simulation` 664/664, require-local packet trace matrix 9/9 local artifacts under `docs/generated/packet-traces/r225-matrix`, `cargo +1.89.0 fmt --check`, and `git diff --check`. At R225 time backend/server tracked slice was 99.70%; R300 later closed the packet gate under explicit stable-diff acceptance.

> Previous sync: R224 completed. The local packet trace blocker closed: `apps/gateway/src/bin/packet_trace.rs` was restored, `--list-flows` worked, `mir2-gateway` passed 53/53 including packet trace bin tests 6/6, and require-local `packet_trace --matrix` wrote 9/9 TCP-traceable artifacts with `localOk=true` under `docs/generated/packet-traces/r224-matrix`. Later Windows rounds closed the server-data import gate and accepted the stable-diff live packet gate.
...
- Active parity round: `2026-04-28-R301`
- Active product-evolution continuation: after `2026-04-28-R256`.
- Active parity state: R248 closed the R39 manifest-backed map-flag import on Windows with local `Server.MirDB` plus matching `Build/Server/Debug/Envir/Routes`; R298 refreshed the stable live Crystal packet matrix evidence slice under `docs/generated/packet-traces/r298-live-matrix` with 9/9 stable diffs clean; R299 proved strict exact dirtiness is Crystal dynamic state; R300 explicitly accepted stable-diff packet parity for the tracked backend/server matrix; and R301 refreshed the final automated Candidate acceptance pack under `docs/generated/player-qa/r301-summary.json`. Strict exact live diff remains diagnostic, and human Crystal visual/feel acceptance is still open.
- Last completed product-evolution round: `2026-04-28-R256`.
- Last completed parity evidence round: `2026-04-28-R301`.
- Backend/server parity status: `100% Accepted under explicit stable-diff packet acceptance`
- Whole-project automation status: `100.0% Candidate`
- Whole-project real accepted 1:1 estimate: `roughly 90.0%` until final human Crystal visual/feel acceptance is closed.
...
- Latest full backend verification: `cargo +1.89.0 test --locked -p mir2-simulation -- --test-threads=1` passed with 674 tests, `cargo +1.89.0 test --locked -p mir2-gateway -- --test-threads=1` passed with 55 tests plus packet-trace bin 15 tests, `cargo +1.89.0 test --locked -p mir2-admin-api -- --test-threads=1` passed with 22 tests, and `cargo +1.89.0 test --locked -p mir2-game-data -- --test-threads=1` passed with 27 tests after R301.
- Latest frontend/global verification: web `tsc --noEmit`, `npm.cmd run build`, minimap smoke with `docs/generated/assets/r301-minimap-assets.json` (0 failures, known 450/451 warning), map API smoke with `docs/generated/map/r301-crystal-map-api.json` (18/18), Stage 5 UI smoke with 88 screenshots including login/select lifecycle, compact layout/text/multi-panel checks, inventory/storage/character/belt/minimap/chat/system-menu flows, Mail/Report/NPC panel state, compact Mail/Report panel bounds, advanced Stage 5 systems state, auction/trade/shop/conquest/hero/mining/craft/mail-delete evidence, map-transfer minimap evidence, and WS load 64/64 ready with 0 errors passed through R301.
...
- Local asset note: Windows now has the server DB and route files needed for the generator; keep the refreshed manifests under test if Crystal DB/routes change again.
- Windows continuation note: after pulling on Windows, read `docs/WINDOWS-CONTINUATION.md`, then continue from `docs/AGENT-TASK-QUEUE.md`. Treat `100.0% Candidate`, backend/server tracked-slice `100% Accepted under explicit stable-diff packet acceptance`, and real full-project accepted 1:1 `roughly 90.0%` as separate metrics. Do not claim full-project 100% Accepted until the human acceptance script passes or the user explicitly accepts remaining differences.
...
- Truth audit note: `docs/PARITY-TRUTH-AUDIT.md` is the authoritative status split for Accepted vs Candidate vs Fallback vs Blocked. In particular, stable live packet matrix evidence is accepted for the tracked backend/server packet gate only; Mac-local synthetic map rendering, JSON account store persistence, product-evolution Admin surfaces, and missing human visual/feel acceptance must not be counted as final accepted Crystal 1:1.
- Admin implementation note: `apps/admin-api` is now an HTTP-capable operations-backend slice with repository traits, in-memory and Postgres command/audit/approval/outbox/operator stores, an admin outbox boundary, `dispatch-admin-outbox`, Axum routes, `SendSystemMail`, grant item/gold, kick player, and ban account executors. `apps/admin-web` is a separate NextJS operations console with login/dashboard/GM/audit/approval/timeline/operators pages wired to Rust. GM system mail is connected to live gateway/account-store Stage 5 mail and is visible/claimable in-game. Activities, Economy price feeds, Risk trade graph, Servers zone telemetry, and Operators/RBAC now have Postgres-backed projection/config tables plus Admin Web write forms. Gateway can automatically post runtime heartbeat rows to `/admin/servers/zones`.
- Persistence implementation note: default gameplay account persistence is still JSON for parity/local safety. `MIR2_ACCOUNT_STORE_DATABASE_URL` mirrors JSON saves into Postgres. `MIR2_ACCOUNT_STORE_BACKEND=postgres` opts into Postgres source-of-truth mode with account row locks and `store_version` / `save_version` increments. It now has stale account writer, stale save writer, and reload-save integration coverage. Account bans persist in the account store and are enforced by simulation login/start-game.
- Cache implementation note: gateway online session caching has `GatewaySessionCache` with in-memory and Redis implementations. Redis is selected with `MIR2_GATEWAY_REDIS_CACHE_URL`; TTL defaults to 30s and can be set with `MIR2_GATEWAY_SESSION_CACHE_TTL_SECONDS`. Redis stores both session records and a character-name routing index with the same TTL for Admin kick-player removal. `GET /admin/sessions` lists current live session records for Admin API read models. Broader reconnect/route-cache semantics remain open.
- Admin production note: Admin API supports strict Postgres operator bearer-token auth through `ADMIN_OPERATOR_AUTH_BACKEND=postgres`, `/admin/auth/me`, token rotation on `POST /admin/operators`, policy-file/static-token bootstrap fallbacks, high-risk command approvals through persistent approval records, default requester self-approval blocking, approval submission matching by command id/type/requester, item/gold grant executors through audited mail delivery, kick/ban command routes, and retry/dead-letter outbox state. Full production external identity provider/session auth, richer approval workflows, support-case tooling, rate limits, deployment hardening, and broader GM executors remain open.
- Event analytics note: Redpanda and ClickHouse run in the local Compose stack. ClickHouse consumes Redpanda topics `admin.command.succeeded`, `admin.command.failed`, `admin.command.denied`, `admin.approval.requested`, `admin.approval.approved`, `admin.approval.rejected`, `admin.outbox.retry`, and `admin.outbox.dead_letter` through `infra/clickhouse/initdb/001_admin_events.sql` and projects events to `mir2_events.admin_events` plus `mir2_events.admin_command_events`. `dispatch-admin-outbox` publishes to Redpanda through `ADMIN_OUTBOX_REDPANDA_URL` and to NATS through `NATS_ADDR`; `ADMIN_OUTBOX_NATS_MODE=jetstream` enables JetStream publish acks. Rows track `nats_status`, `redpanda_status`, `last_error`, and `dispatched_at_ms`, and are marked dispatched only after all configured publishers succeed. Admin API `/admin/events` and `/admin/timeline` read the ClickHouse projection with degraded responses. NATS is still the current admin outbox notification dispatcher; Redpanda/ClickHouse are not gameplay source of truth.

## Last Completed Round: R301

R301 refreshed the final automated Candidate acceptance pack after R300:

- Evidence summary is `docs/generated/player-qa/r301-summary.json`.
- Packet acceptance remains the R300 stable-diff decision; strict exact is still diagnostic.
- Web verification passed `tsc --noEmit` and `npm.cmd run build`.
- Map API smoke passed 18/18 with 0 failures; minimap smoke passed with 0 failures and the known 450/451 warning.
- Stage 5 UI smoke captured 88 screenshots, 0 critical console errors, and checked 32 compact text nodes with no overflow.
- WS load passed 64/64 ready with 0 errors and keepalive p95 637 ms.
- Rust verification passed `mir2-game-data` 27/27, `mir2-gateway` 55/55 plus packet-trace bin 15/15, `mir2-admin-api` 22/22, and `mir2-simulation` 674/674.
- Temporary gateway/web services were stopped and ports 7000/7110/3002 verified closed.

## Previous Completed Round: R300

R300 made stable-diff packet acceptance explicit and enforceable:

- `apps/gateway/src/bin/packet_trace.rs` now separates strict exact diagnostics from accepted packet parity.
- `MIR2_PACKET_TRACE_ACCEPT_STABLE_DIFF=1` selects stable-diff acceptance; `MIR2_PACKET_TRACE_REQUIRE_CRYSTAL=1` / `MIR2_PACKET_TRACE_REQUIRE_ACCEPTED_DIFF_CLEAN=1` enforce the configured accepted comparator.
- Matrix summaries include `acceptanceMode`, `acceptedPacketParityCount`, and `packetParityAccepted`.
- R298's 9/9 stable-clean live Crystal matrix plus R299's payload probe are recorded as the accepted packet parity evidence in `docs/PACKET-PARITY-ACCEPTANCE.md` and `docs/generated/packet-traces/r300-stable-acceptance.json`.
- Verification passed: `cargo +1.89.0 test --locked -p mir2-gateway --bin packet_trace -- --test-threads=1` (15/15) and `cargo +1.89.0 fmt --check`.

## Previous Completed Round: R298

R298 refreshed live Crystal stable packet-matrix evidence on Windows:

- Live Crystal `127.0.0.1:7000`, local gateway `127.0.0.1:7310`, `CRYSTAL_CLIENT_ROOT=E:\mir2\Crystal\Build\Client\Debug`, and fixture character `Cdx0428030348` index `8` produced `docs/generated/packet-traces/r298-live-matrix/latest-matrix.json`.
- Matrix summary: `artifactCount=9`, `localOkCount=9`, `crystalOkCount=9`, `crystalMissingCount=0`, `stableDiffCleanCount=9`, `acceptedStableLiveComparisonCount=9`, `diffDirtyCount=9`, and `acceptedLiveComparisonCount=0`.
- The packet-trace stable comparator now treats Crystal `TimeOfDay` as volatile; strict exact live diff remains dirty and is diagnostic after R300's stable-diff acceptance decision.
- Verification passed without Docker: `mir2-simulation` 674/674, `mir2-gateway` 55/55 plus packet-trace bin 14/14, `mir2-admin-api` 22/22, `fmt --check`, `git diff --check`, and web `tsc --noEmit`.

## Previous Completed Round: R297

R297 refreshed Windows frontend/player QA automation and fixed blockers found during that run:

- Account-store JSON saves serialize/retry atomic replacement to avoid Windows concurrent-load failures.
- WS load creates a character before `StartGame`, matching Crystal-style empty new accounts.
- Gateway `MapInformation` now exposes minimap/big-map indices so map-transfer minimap state updates.
- Missing original scene sprite libs were exported from Crystal `Data`.
- Validation passed web build/typecheck, map API smoke 18/18, minimap smoke 0 failures with known 450/451 warning, WS load 64/64 ready with 0 errors, Stage 5 UI smoke 88 screenshots with 0 critical console errors, `mir2-simulation` 674/674, `mir2-gateway` 55/55 plus packet-trace bin 14/14, `mir2-admin-api` 22/22, `fmt --check`, and `git diff --check`.

## Previous Completed Round: R224
...
- Live Crystal stable diff is available from R298; strict exact live diff remains dirty and is diagnostic after R300's stable-diff acceptance decision.
...
- R298 ran `cargo +1.89.0 run --locked -p mir2-gateway --bin packet_trace -- --matrix` against live Crystal; stable live matrix evidence passed with 9 artifacts and 17 intentionally skipped non-TCP matrix entries. R300 later accepted/enforced the stable-diff comparator for the tracked backend/server packet matrix; strict exact remains diagnostic.
