...
> Latest product-evolution sync: 2026-04-28-R248 completed. Admin Web now uses true Rust Admin API read models instead of `mock-data.ts` for Dashboard, Players, Player Detail, Economy, Activities, Servers, and Risk. `/admin/read/*` derives player/economy/hot-map/risk data from JSON account-store or explicit Postgres source mode; activity config, market prices, trade graph, zone runtime, and online-session totals show empty/unwired states until their authoritative projections are added.
