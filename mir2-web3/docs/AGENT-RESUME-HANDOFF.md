...
- Active product-evolution continuation: after `2026-04-28-R253`.
...
- Last completed product-evolution round: `2026-04-28-R253`.
...
- Admin production note: Admin API supports optional bearer operator token auth through `ADMIN_OPERATOR_TOKEN`, policy-file operator auth through `ADMIN_OPERATOR_POLICY_PATH`, Postgres operator/RBAC records for the local control plane, high-risk command approvals through persistent approval records, default requester self-approval blocking, item/gold grant executors through audited mail delivery, kick/ban command routes, and retry/dead-letter outbox state. Full production operator identity provider/session auth and multi-approver policy remain open.
