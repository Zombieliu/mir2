...
- Admin implementation note: `apps/admin-api` is the first operations-backend code slice. It currently contains command/audit/RBAC primitives and in-memory tests only; it is not yet an HTTP API and is not wired to live game state.
