...
- Admin implementation note: `apps/admin-api` is now an HTTP-capable operations-backend slice with repository traits, in-memory command/audit stores, Axum routes, and a `SendSystemMail` domain outbox executor. `apps/admin-web` is a separate NextJS operations console with the first desktop UI pages and a Next proxy route wired to Rust for GM system mail. It is still not connected to live account/world/mail game state, Postgres, or real operator auth.
