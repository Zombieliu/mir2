...
- Active product-evolution continuation: after `2026-04-28-R246`.
...
- Last completed product-evolution round: `2026-04-28-R246`.
...
- Truth audit note: `docs/PARITY-TRUTH-AUDIT.md` is the authoritative status split for Accepted vs Candidate vs Fallback vs Blocked. In particular, stable live packet matrix evidence is accepted for the tracked backend/server packet gate only; Mac-local synthetic map rendering, Admin mock read models, JSON account store persistence, and missing human visual/feel acceptance must not be counted as final accepted Crystal 1:1.
- Admin implementation note: `apps/admin-api` is now an HTTP-capable operations-backend slice with repository traits, in-memory and Postgres command/audit/approval/outbox stores, an admin outbox boundary, `dispatch-admin-outbox`, Axum routes, `SendSystemMail`, grant item/gold, kick player, and ban account executors. `apps/admin-web` is a separate NextJS operations console with dashboard/GM/audit/approval/timeline pages wired to Rust. GM system mail is connected to live gateway/account-store Stage 5 mail and is visible/claimable in-game.
...
- Cache implementation note: gateway online session caching has `GatewaySessionCache` with in-memory and Redis implementations. Redis is selected with `MIR2_GATEWAY_REDIS_CACHE_URL`; TTL defaults to 30s and can be set with `MIR2_GATEWAY_SESSION_CACHE_TTL_SECONDS`. Redis now stores a character-name routing index with the same TTL for Admin kick-player removal. Broader reconnect/route-cache semantics remain open.
- Admin production note: Admin API supports optional bearer operator token auth through `ADMIN_OPERATOR_TOKEN`, policy-file operator auth through `ADMIN_OPERATOR_POLICY_PATH`, high-risk command approvals through persistent approval records, default requester self-approval blocking, item/gold grant executors through audited mail delivery, kick/ban command routes, and retry/dead-letter outbox state. Full production operator identity provider and multi-approver policy remain open.
