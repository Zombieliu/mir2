...
| WN-ITEM-002 | P0 | Item icons | Every carried/belt/equipment item in the audited slice renders its exact source icon, count, durability overlays, and unavailable state. | Full 1,628-row/924-image source asset coverage remains gated, with all 360 prior PNGs preserved. Source UserItem.Image quantity selection now drives personal bag/belt/equipment/storage and NPC goods, including true-size changes and stale ordinary-icon correction; Windows 527, game-data 39 and real stack integration 4 pass. A historical initial PNG matches 24 fixed bag cells/6,161 opaque samples with negative controls, but predates the final ordinary-icon correction and includes no manual transitions. Earlier same-EXE MirArmour icon/real-unequip evidence is unchanged. Guild/trade/other actual-item surfaces, FloorItems, count/alpha/durability/locked/selected/sealed/unavailable overlays, final-source real input, DPI and human comparison remain open. | Retain all 924 original images; finish remaining concrete-item surfaces without changing valid base-image previews; capture final-source manual stack transitions/overlays after user resumes foreground QA, then trusted same-state/DPI/human comparison. |
