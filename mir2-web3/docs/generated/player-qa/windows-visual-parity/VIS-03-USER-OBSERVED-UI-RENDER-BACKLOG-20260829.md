...
| WN-WORLD-003 | P0 | Environmental animation | Tile, middle, and front animations advance with Crystal frame counts/ticks; lamp flames and fires visibly animate. | Bounded implementation now parses Type100 tile-animation fields, expands complete tile/middle/front animation families, advances a 100 ms Crystal-compatible clock with per-family tick dwell, and holds the base frame when a packaged family is incomplete. Runtime 205/205 and map-parser 31/31 tests pass. Exact Candidate timed capture, additive/blend fidelity, full asset-family audit, soak, and human acceptance remain open. | Timed capture proving at least two source-correct frames, phase/tick unit tests, full visible animation-family residency, and soak stability. |
...
| WN-QUEST-001 | P0 | Quest availability/turn-in markers | NPCs show source-correct `!`, `?`, or equivalent Crystal marker states for available, active/incomplete, and ready-to-turn-in quests; markers update without relog. | User reported no NPC head guidance marker. Current head `ae799f70fcca` now has a bounded native overlay baseline: authoritative `questIds` + `QuestTracker` drive `!` / `?` markers above NPC nameplates with focused Windows tests green. Same-EXE captures, full state-transition coverage, and human visual acceptance remain open. | State-transition matrix, NPC/object binding test, occlusion/z-order test, focused Windows overlay tests, and same-EXE captures for every state. |
