...
| WN-QA-002 | P2 | One-shot capture releases UI control | A completed automatic screenshot must stop preparing its target so subsequent manual UI actions remain possible. | The optional `MIR2_NATIVE_CAPTURE_AUTO_SCREEN` path currently prepares its target before checking `auto.done`; it can keep reopening Character after the one-shot capture. Manual interaction runs disable that option. This does not affect ordinary launches without the QA option. | Move/check completion before preparation, regression for post-capture panel switching, and same-EXE manual interaction after auto capture. |
...
| WN-ACTOR-001 | P0 | Player animation integrity | Idle/walk/run/direction/composite layers remain coherent, do not flicker, and use Crystal cadence. | Several bounded fixes passed tests and the user later reported direction animation improved, but sustained-run flash/cadence and full action/class denominator remain open. The September 3 wing QA also visibly exposes head-only world actors with HeavenArmour/MirArmour even while CharacterDialog's separate StateItem paper doll renders correctly. The world-actor asset/layer path must be audited independently. | Long movement trace, frame-sequence and complete resident-layer assertions for every in-scope direction/action/class/equipment set, same-EXE video, and human feel acceptance. |
...
| WN-ITEM-002 | P0 | Item icons | Every carried/belt/equipment item in the audited slice renders its exact source icon, count, durability overlays, and unavailable state. | Source-sized/centred starter bag icons and stack-count rules are implemented and present in the current same-EXE run. The wing matrix identifies two concrete missing exports: MirArmour(M) needs `Items/595.png` and MirArmour(F) needs `Items/605.png`; both source catalogue rows exist but these PNGs are absent, so the worn-icon cells remain empty in the r3 captures. Complete audited item-manifest coverage, durability/unavailable overlays, DPI and human comparison remain open. | Export/verify the exact missing original frames without substitutes; item-manifest coverage, missing-asset fail-closed test, representative grid screenshot and complete audited-slice count. |
...
| WN-CHAR-001 | P1 | Character paper doll | CharacterDialog uses gender-specific page art and composes wing, resolved armour, weapon, helmet-or-hair and 14 equipment cells in Crystal draw order with each `.Lib` frame's source offset. | The missing data/composition chain is implemented: source name/guild header, gender page art, exact StateItem/hair layers and fourteen cells were captured on September 2. September 3 adds authoritative real-armour WingEffect, explicit-zero clearing, source Prguse2 1202..1205 geometry and additive DrawBlend before armour/weapon/helmet-or-hair, with four same-EXE gender/effect captures. Full class/hair/helmet/equipment coverage, the MirArmour icons in WN-ITEM-002, other Character tabs, trusted Crystal pairs and human acceptance remain open. | Retain runtime/source/export/layer/order/offset regressions; finish the representative class/gender/equipment matrix, same-state same-EXE screenshots and human acceptance. |
