...
## 2026-09-03 Guild storage source/logic checkpoint
...
| WN-ITEM-001 | P1 | Inventory/belt geometry | Inventory and belt cell size, spacing, frame, padding, clipping, footer and icon fit match Crystal. | Bag1/Bag2/QuestInventory use source `316x236` window/grid geometry, footer values/states and a source movable-window path with real-pointer before/after evidence. Exact primary icon centring is reopened: the 2026-09-03 source alpha audit finds 550/1003 frame-size differences and 478 changed 35-pixel-cell offsets; only the new Guild/coin helper uses GetTrueSize. Earlier starter/stack PNG checks used frame dimensions and do not prove Crystal true-size centring. Belt-wide geometry, trusted same-state comparison and DPI/human acceptance remain open. | Correct primary icon offsets using source alpha-bound size while drawing the full bitmap; preserve source grid/footer evidence, add actual-PNG regressions, same-item original pairs, trusted exact-EXE evidence and 100/125/150% DPI checks. |
...
| WN-ITEM-002 | P0 | Item icons | Every carried/belt/equipment item in the audited slice renders its exact source icon, count, durability overlays, and unavailable state. | Full 1,628-row/924-image source asset coverage remains gated, with all 360 prior PNGs preserved. Source UserItem.Image drives primary bag/belt/equipment/storage and NPC goods; the new Guild 8x8 viewport uses current counts, alpha-bound GetTrueSize centring, full unstretched PNGs, Yellow stack counts and legitimate Items/0 with 551 shared native tests and 41 direct source-frame checks. Primary paths still need alpha-size centring and zero-sentinel fixes. The historical PNG matches 24 fixed bag cells/6,161 opaque samples against PNG-frame geometry, predates the final ordinary-icon correction and has no manual transitions; it does not prove original true-size centring. Earlier same-EXE MirArmour/unequip evidence is unchanged; no current Guild GUI capture exists. Guild operations/state overlays, trade/other surfaces, FloorItems, final-source real input, DPI and human comparison remain open. | Retain all 924 original images and the full 1003-frame audit; correct primary GetTrueSize/zero-image handling, finish concrete-item paths and source operations/overlays without changing valid base-image previews; capture final-source manual transitions after user resumes foreground QA, then trusted same-state/DPI/human comparison. |
| WN-ITEM-003 | P1 | Item detail tooltip | Hover shows source-correct name, type, weight, durability, stats, requirements, bind/trade restrictions, description, price, and comparison where Crystal does. | Bounded personal-item implementation is green: exact/real item plus recursive socket/user metadata and player stats drive eleven source sections on inventory, belt, equipment, personal storage and warehouse-side bag; disabled cells stay hoverable, expiry/seal/rental text and `+(28,28)` clamping are tested, with real-pointer same-EXE WoodenSword evidence. NPC shop has source MirGoodsCell layout/HideAddedStats and populated selected/hover capture, but its PNG-frame icon centring still needs source GetTrueSize. Guild now has source grid/scroll geometry, alpha-centred current-instance icons/counts and the common hint, but still lacks a final-source populated GUI capture and complete item operations/overlays. GameShop, quest rewards and trade still need exact layouts and captures; duplicate/sub-goods topology, soul-bound names and Hero requirement 129 remain open. | Close remaining specialized geometry/topology including primary/NPC alpha-size centring and Guild/other populated captures; add specialized-item matrix, full text/overlap behavior, trusted package/light plus 100/125/150% DPI captures, and human same-state Crystal comparison. |
...
| WN-CHAR-001 | P1 | Character paper doll | CharacterDialog uses gender-specific page art and composes wing, resolved armour, weapon, helmet-or-hair and 14 equipment cells in Crystal draw order with each `.Lib` frame's source offset. | The missing data/composition chain is implemented: source name/guild header, gender page art, exact StateItem/hair layers and fourteen cells were captured on September 2. September 3 adds authoritative real-armour WingEffect, explicit-zero clearing, source Prguse2 1202..1205 geometry and additive DrawBlend before armour/weapon/helmet-or-hair. The later full-icon export and four new same-EXE cases also restore the previously empty MirArmour worn cells. Full class/hair/helmet/equipment coverage, other Character tabs, trusted Crystal pairs and human acceptance remain open. | Retain runtime/source/export/layer/order/offset regressions; finish the representative class/gender/equipment matrix, same-state same-EXE screenshots and human acceptance. |
