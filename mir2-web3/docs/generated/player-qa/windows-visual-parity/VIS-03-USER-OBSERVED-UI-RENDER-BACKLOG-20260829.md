...
| WN-ITEM-002 | P0 | Item icons | Every carried/belt/equipment item in the audited slice renders its exact source icon, count, durability overlays, and unavailable state. | Complete source asset coverage is now gated: all 1,628 catalogue rows, 913 base Images plus eleven Amulet/Poison quantity images, 924 unique required PNGs; 643 missing files were added without substitutes and 360 prior PNGs preserved. The gate verifies catalogue/source/geometry/pixel/file identities and rejects omitted metadata/files. Same-EXE captures show both MirArmour worn icons and Items/595 moving into first bag cell after real unequip. Runtime quantity-dependent image selection/live stacks, durability/locked/sealed/unavailable overlays, DPI and human comparison remain open. | Retain the full 924-image union and exporter failure tests; implement/test source UserItem.Image threshold selection across item surfaces, capture live stack transitions and overlays, then trusted same-state/DPI/human comparison. |
