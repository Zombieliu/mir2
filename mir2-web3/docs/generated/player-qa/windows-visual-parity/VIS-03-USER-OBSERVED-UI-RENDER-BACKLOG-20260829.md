...
| WN-INPUT-003 | P0 | NPC hover/interact | NPC cursor, approach, range, single-open behavior, dialog, and cancellation mirror Crystal. | A bounded approach bridge exists, and current head now also preserves NPC hover/clickability when body alpha pixels are temporarily unavailable by falling back to the NPC body bounds only for NPCs; monster targeting stays fail-closed. Focused hover and approach tests are green, but exact dialog capture, live retest, and Crystal `DataRange` behavior remain open. | Near/far/blocked/double-click tests plus exact dialog screenshot and no duplicate request. |
| WN-QUEST-001 | P0 | Quest availability/turn-in markers | NPCs show source-correct `!`, `?`, or equivalent Crystal marker states for available, active/incomplete, and ready-to-turn-in quests; markers update without relog. | Authoritative `questIds` + `QuestTracker` now select Crystal's real `Prguse` pairs: white question `983/984`, yellow exclamation `985/986`, and yellow question `987/988`, with the original 500 ms two-frame cadence and ready > available > in-progress priority. Focused Windows tests pass. Exact body-frame anchoring, same-EXE captures, complete live state transitions, occlusion/z-order review, and human acceptance remain open. | State-transition matrix, NPC/object binding test, exact body-frame anchor and occlusion/z-order tests, focused Windows overlay tests, and same-EXE captures for every state. |
