...
## 2026-09-03 Accepted-exchange TradeDialog source/logic checkpoint

WN-ITEM-002/003 now include original own/guest 204x152 windows, ten fixed sparse
cells per side, source controls and the gold MirAmountBox. Independent bag,
basic pair dragging, current-count icons/hints, exact own-offer identity,
exchange/unlock revisions and source completion/unlock/cancel have headless
coverage. Escape follows source Closeall and does not cancel the pair.

Native UI 591/591, Windows 534/534, runtime 212/212, UI core 43/43 and the
11-test/924-image gate pass. The original asset verifier matches 561 PNGs,
adds five original frames, preserves all 552 prior Prguse/Title PNG bytes
and rejects 32 negative controls. Report:
`../native-ui-parity-20260903-trade-dialog/README.md`.

This is not full trade: both grids remain read-only; legacy first-ten-bag/
to=0 shortcuts were removed rather than accepted as source operations.
Original invitation/cancel messages, Gold 106.wav, complete text editing,
state overlays and all-window input/topmost remain open. Source inspection
also confirms Candidate sends S.TradeConfirm before mutual/durable settlement
and replaces TradeGold instead of source positive-delta/immediate escrow.
The next CLI leaf is transaction phases/conservation, followed by exact
native item operations and raw-versus-normalized slot contracts. Backend
code is unchanged here; no new full-server test outcome is inferred.

Computer Use remains paused after user Escape; no GUI/input/capture occurred.
Final-source same-EXE original pairs, package/light, actual DPI, soak, legal/
signing and human gates remain. All 33 IDs are preserved; `visualAccepted=false`,
`accepted=false`, `globalParityPercent=null`.

## 2026-09-03 Primary-item true-size source/logic checkpoint

WN-ITEM-001/002/003 now include alpha-bound GetTrueSize/full-bitmap placement
for bag, belt, equipment, personal storage and NPC goods, plus source Items/0
and StackSize-driven count 1. WN-CHAR-001's fourteen equipment cells and the
personal storage grid now use original 36x32 bounds; oversized item art is
not clipped to cells, bag/grid or NPC rows. WN-HUD-001's belt leaf handles
late loads, current-count image changes and clearing without stale/white art.
Warehouse-side, trade and amount icon regions use the same rule, without
accepting their outer windows, operations or state overlays.

The original verifier compares all 1003 exported PNGs against Items.Lib RGBA
and metadata. The complete source fixture reproduces exactly, and native
tests check 5015 node geometries across five cell sizes. Full native UI
562/562, Windows 528/528, runtime 212/212, UI core 43/43, item-icon 11/11 with
all 924 required images and formatting pass. These are source/pixel-asset/
headless-node checks, not new GPU or same-EXE screenshot evidence.
Report: `../native-ui-parity-20260903-item-true-size/README.md`.

Computer Use remains paused after user Escape; no application window, input
or capture occurred. This supersedes only the primary centring/zero-image
gaps below. Trade/other specialized layouts, source operations, state
overlays, FloorItems and the original-pair/package/light/DPI/soak/human gates
remain open. All 33 IDs are preserved; `visualAccepted=false`,
`accepted=false`, `globalParityPercent=null`.

## Historical 2026-09-03 Guild storage source/logic checkpoint

WN-ITEM-002/003 now include the original eight-column Guild storage grid,
fourteen authoritative rows and eight visible rows; scrolling retains all
112 original slot IDs, the initial index/viewport asymmetry and exact integer
drag/wheel steps. Current-count UserItem.Image and alpha-bound GetTrueSize
replace fixed stretched base images; the full PNG still draws without alpha-
origin/library offsets, and concrete Items/0 remains drawable on this path.
Source tabs/arrows and MirAmountBox gold input replace generic controls.
Gold uses existing pending requests and rechecks identity/rank/balance without
optimistic mutation; modal, focus, session and rapid-input cases are tested.

Final native UI 551/551, focused Guild 44/44, Windows 527/527, runtime 212/212,
stack integration 4/4, UI core 43/43, protocol 40/40, game-data 39/39,
the 11-test/924-image gate and 41 direct original RGBA/geometry checks pass.
Current host/runtime/integration results and source
hashes: `../native-ui-parity-20260903-guild-storage/README.md`.

All 1,003 exported Items were audited against source alpha bounds: 550 PNG
frame sizes differ from GetTrueSize and 478 change 35-pixel-cell centring.
Only the new Guild/coin path is corrected here. Primary bag/belt/equipment/
storage/NPC icon paths and their Items/0 sentinel remain open. Older fixed-
coordinate PNG tests prove their stated image identity, not original true-
size centring; this finding supersedes that part of earlier geometry claims.

No application EXE was launched, input injected or native screenshot captured:
Computer Use remains paused after user Escape. Guild item store/retrieve/merge
UI, state overlays, full text/clipboard/GDI behavior, window movement/topmost
pointer dispatch and cross-action throttle remain open. Trade/other actual-item
surfaces, FloorItems and all final-source original-pair/package/light/DPI/human
gates remain. All 33 existing IDs are preserved; no overall acceptance change.

## 2026-09-03 Source quantity-image projection checkpoint

WN-ITEM-002 now has the exact source UserItem.Image selector on primary
bag/belt/equipment/storage and NPC-goods paths. Current quantity drives all
eleven original frames and their true-size geometry; known ordinary items
...
pixels, rejecting 86 wrong source frames and 96 one-pixel translations.
Count-label pixels, alpha edges and other panels are excluded. Both captured
binaries predate the final ordinary-item icon correction; they are not a
fresh final-source same-EXE evidence set. The user stopped Computer Use with
Escape before manual split/merge/equip/storage/shop transitions; do not
resume foreground automation without an explicit user request.

Evidence: `../native-ui-parity-20260903-item-stack-images/README.md` and its
separate process/hash ledger. Guild/trade/other actual-item surfaces,
FloorItems, overlays, real input and all paired-state/trusted/DPI/human gates
remain open. WN-CHAR-002 and every other prior backlog ID are preserved.
This supersedes the older blanket missing-runtime-selector note only for
the named primary surfaces, not the full WN-ITEM-002 denominator.

## 2026-09-03 Item-icon/high-armour and one-shot-release checkpoint

All 1,628 catalogue rows now retain their 913 base Images plus eleven original
Amulet/Poison count-dependent images. The mandatory denominator is 924 unique
images, not only the two first observed missing MirArmour icons. The exporter
adds 643 source PNGs, verifies/reuses all 360 older PNGs byte-for-byte, and
records source-library/catalogue plus decoded-pixel/PNG fingerprints. The
full CArmour/09 and /10 libraries add 3,232 original body frames; all pass
direct original-pixel/geometry comparison, and 512 Warrior stand/walk/run
gender/direction composites keep body, hair and weapon resident.

Fresh EXE `55CA1D61A6977F164B1D6222DE2AAAFD4E21FEF4A1C5DB898EBEE6FF0FBDB993`
produces four gender/armour captures and a real auto-Character -> manual
Inventory -> unequip -> first-bag-icon -> re-equip cycle. WN-QA-002 now checks
completion before preparing its target and passes the App regression plus
live post-auto panel switching. The fixture's second bag is locked; the live
sidecar records page 0 and is not claimed as page-two evidence.

Windows 524/524, icon tests 11/11 plus 924-image closure, exporter regressions,
and the ten-control Windows PowerShell 5.1 gate self-test pass. Full frontend
logic is still incomplete because the local WebGPU WASM artifact is absent;
its five later checks pass separately. Report and unchanged renderer-owned
sidecars: `../native-ui-parity-20260903-item-actor-assets/README.md`.

Runtime quantity-image selection, world CHumEffect wings, full class/action/
equipment coverage, source item operations and all original-pair/DPI/trusted
provenance/human gates remain open. New WN-ACTOR-002 keeps the visible
name-only previous-player observations from rapid QA process switches in the
denominator. All claim-state flags above stay false/null; older checkpoints
are historical rather than current missing-asset assertions.

## 2026-09-03 Character wing and unequip checkpoint

Current branch `codex/windows-player-journey` adds source-derived self
WingEffect and exact `Prguse2/1202..1205` geometry/DrawBlend before the existing
StateItem layers. Crystal source is pinned at
`92b4ce4ab488b11e65f63d3ad22de2e1f25ec08d`; the relevant source files are clean.
The final EXE SHA-256 is
`8BA170AA654FCE1EB911033203C22F018EAD2A4B973F04A4F80A8679E2FF6F20`.
Four gender/effect cases and the same-process WingOneM `(290,620)` real-pointer
wing -> no wing -> first bag cell -> restored wing cycle are recorded in
`../native-ui-parity-20260903-character-wings/README.md`. Sidecars are unchanged
renderer-owned drafts with truthful missing light/trusted-build blockers.

Native UI 519/519, runtime 212/212, Windows 521/521, Simulation 1491/1491,
Gateway 666 active/1 ignored, wing integration 5/5 and affected recall
integration 2/2 pass. The Archer CI regression now uses its own deterministic
manifest and tests the unavailable-library fallback without changing
production routing. New WN-CHAR-002 and WN-QA-002 retain the still-open source
item-operation and one-shot capture gaps. WN-ITEM-002 and WN-ACTOR-001 also
record high-armour missing-icon/world-layer observations from the expanded
matrix. The denominator is expanded, not reduced; all claim-state flags above
remain false/null. The older recorded branch/head block is historical, not
the identity of these new captures.
...
| WN-QA-001 | P0 | Canonical same-state visual fixture | Crystal and Candidate captures bind the same visible character state: character/class/gender/hair/level, canonical map/coordinates/direction/light, HP/MP/EXP, Gold, belt/bag/equipment, quest inventory/progress/completed IDs and open-panel state. Credentials and source account identity are redacted. | Still open, but the original extractor failure is superseded: extraction/upsert now retains hair, quest inventory/progress/completed IDs and maps raw bag indexes after the six belt cells; `test-crystal-account-state.mjs` passes 9/9. The September 2 Character/Inventory/Quest evidence uses an isolated import of Crystal `1231`, not `demo/Scout`. Capture sidecars still lack authoritative light and trusted package identity; the new wing matrix uses separate explicitly labelled catalogue QA characters and is not a same-state Crystal pair. | Redacted canonical-state JSON plus hash in both sidecars, fail-closed pair verification, authoritative light/build provenance, then three same-state Character/Inventory/Quest pairs. Preserve the complete fixture regression denominator. |
| WN-QA-002 | P2 | One-shot capture releases UI control | A completed automatic screenshot must stop preparing its target so subsequent manual UI actions remain possible. | Implemented and bounded-verified September 3: completion is checked before preparation; the Bevy App test preserves manual Inventory page/tab and later notice state over three updates. The same EXE keeps auto-Character enabled, then accepts manual Inventory, unequip and re-equip; the captured Inventory page is 0 because this fixture has no expansion. Ordinary launches without the QA option remain unaffected. | Retain early-completion and App regressions and eight-image same-EXE ledger. This bounded QA leaf does not promote draft sidecars or close original-pair, light, package or DPI acceptance. |
...
| WN-ACTOR-001 | P0 | Player animation integrity | Idle/walk/run/direction/composite layers remain coherent, do not flicker, and use Crystal cadence. | The concrete HeavenArmour/MirArmour head-only self-body hole is fixed by complete original CArmour/09 and /10 exports (3,232 frames), with direct pixel/geometry checks, 512 Warrior gender/direction/stand/walk/run composites and four live cases. The separate world CHumEffect wing layer is still absent despite working CharacterDialog wings. Sustained-run flash/cadence, every other action/class/equipment combination and human feel remain open. | Retain complete body-library and composite regressions; finish source world-wing layers, long movement trace, every in-scope direction/action/class/equipment set, same-EXE video and human feel acceptance. |
| WN-ACTOR-002 | P1 | Remote-player appearance and leave/reconnect visibility | Each visible player retains its authoritative body/equipment/name until the source-compatible leave/remove lifecycle clears it; reconnect grace must not produce unexplained name-only actors. | Rapid switches among the September 3 isolated QA clients leave the prior player's name without a visible body in some initial renderer captures (WingTwoM, WingTwoF, WingOneF). Later observations differ; no packet/tick trace yet distinguishes reconnect grace, appearance projection and renderer cleanup. Restored self bodies do not prove remote-player parity. | Two-client authoritative join/equipment/leave/reconnect transcript with exact EXEs; ObjectPlayer/Remove and AOI timing tests; complete remote resident-layer and name cleanup assertions; same-state video. |
...
| WN-QUEST-003 | P1 | Quest diary/tracker/map guidance | Quest button, diary tabs, objective text, navigation hints, target/map markers, abandon/share rules, and empty/error states match Crystal. | Source-shaped Diary, independent Detail, NPC Quest List, ordered five-slot right-click tracking and CANCEL -> source confirmation -> NO preservation are implemented with the September 2 q2/q3 real-input captures and 52 focused tests recorded in `FRONTEND-1TO1-GAPS.md`. This supersedes the old unrelated-dirty-draft note. Reward/credit art, full guidance/control matrix, tracker persistence, complete capture-sidecar state and human acceptance remain open. | Per-control behavior matrix, known quest snapshots, map/NPC link tests, persistence and exact-state screenshot pairs for all remaining leaves. |
| WN-QUEST-004 | P1 | Quest surface topology | Q opens source-sized `Prguse/961` QuestDiary grouped by `Group`; row left-click opens the separate `Prguse/960` QuestDetail, right-click toggles one of at most five tracked quests, and NPC quest operations use the separate `Prguse/950` QuestList. | The Mail-frame/five-filter substitute has been removed. `group`/`minLevelNeeded` survive the adapters; Diary, Detail, NPC List and frameless Tracking now have distinct state, source assets and bounded geometry/behavior coverage. The September 2 Diary/Detail and q2/q3 captures exercise the split, including closing Diary without closing Detail. Trusted same-state Crystal pairs, remaining reward art/interaction details and human acceptance are still open. | Retain the source-field, asset, ownership and geometry regressions; add trusted same-state same-EXE screenshot pairs and human acceptance. |
| WN-HUD-001 | P1 | Main HUD geometry | Orb HUD, belt, chat strip, experience/weight, minimap, labels, right controls, and scaling use Crystal bounds and ordering. | Whole-HUD drift remains open. The 2026-09-03 primary-image checkpoint corrects the 32x32 belt's alpha-size centring, current-count/source-zero images, source stackable count 1 and late-load/change/clear ordering, with actual-PNG and default-white/stale-image regressions. No fresh GUI capture exists while Computer Use is paused. | Retain the belt source/PNG regressions; finish whole-HUD geometry and ordering at 1024x768, real 100/125/150% DPI captures and same-scene human comparison. |
...
| WN-ITEM-001 | P1 | Inventory/belt geometry | Inventory and belt cell size, spacing, frame, padding, clipping, footer and icon fit match Crystal. | Bag1/Bag2/QuestInventory retain source 316x236 grid/footer and movable-window evidence. The 2026-09-03 primary-image checkpoint now centres with alpha-bound GetTrueSize, draws full original PNGs without cell/bag/grid clipping and keeps the belt at source 32x32. All 1003 exported images match source RGBA/metadata and 5015 native node geometries pass. Earlier PNG-frame captures are historical, not final-source true-size or same-EXE evidence. Whole-belt/HUD, trusted original pairs and DPI/human acceptance remain open. | Retain source grid/footer, full-PNG and node regressions; capture final-source manual transitions and same-item original pairs after explicit Computer Use resumption, then trusted exact-EXE and 100/125/150% DPI/human checks. |
...
| WN-ITEM-002 | P0 | Item icons | Every carried/belt/equipment item in the audited slice renders its exact source icon, count, durability overlays, and unavailable state. | The 1628-row/924-image gate and 1003-export/5015-node fixture remain intact. Primary/Guild current-count UserItem.Image, valid Items/0, alpha-bound full-bitmap placement and StackSize count 1 extend to the source-shaped own/guest trade grids with ten sparse slots each, exact authoritative own-offer identity and shared hints. Native UI 591/591 and Windows 534/534 pass; 561 trade/modal asset checks preserve all 552 prior Prguse/Title PNGs. Legacy zero without source stays absent. Trade cells are read-only, invitations and full transactions remain open, and no final-source GUI capture exists. Source operations/overlays, FloorItems and all prior human gates remain. | Preserve all image/slot denominators; fix premature shared TradeConfirm and TradeGold delta/escrow semantics with conservation tests, then complete exact native item operations and remaining surfaces/overlays without changing base-image previews. After explicit foreground resumption, capture real final-source transitions and trusted original-pair/DPI/human comparisons. |
| WN-ITEM-003 | P1 | Item detail tooltip | Hover shows source-correct name, type, weight, durability, stats, requirements, bind/trade restrictions, description, price, and comparison where Crystal does. | Personal source/recursive instance sections, clamping and disabled hints retain existing tests and historical WoodenSword evidence. NPC MirGoodsCell/HideAddedStats keeps its historical capture; actual-PNG tests retain corrected primary/NPC geometry. Guild has source grid/scroll/hints. Trade now has original own/guest geometry, sparse exact-identity cells and shared hints, but read-only cells, invitation/transaction gaps and no populated final-source capture. GameShop/quest rewards, duplicate/sub-goods topology, complete operations/overlays, soul-bound names and Hero requirement 129 remain open. | Retain primary/NPC/Guild/trade source and hint regressions; finish transaction phases, exact source item-operation matrices and other specialized topology. Then verify populated final-source captures, full editing/overlap, trusted package/light, real 100/125/150% DPI and human same-state Crystal comparison. |
...
| WN-CHAR-001 | P1 | Character paper doll | CharacterDialog uses gender-specific page art and composes wing, resolved armour, weapon, helmet-or-hair and 14 equipment cells in Crystal draw order with each `.Lib` frame's source offset. | Source name/guild header, gender page art, StateItem/hair composition and fourteen cells retain September 2 captures. September 3 adds real-armour WingEffect, explicit-zero clearing and source additive wing geometry before armour/weapon/helmet-or-hair; later icon exports restore the MirArmour worn cells in historical same-EXE cases. The latest primary-image checkpoint corrects all fourteen hit cells from 32x32 to original 36x32 and alpha-centres full PNGs with headless coverage. No new final-source capture exists. Full class/hair/helmet/equipment coverage, other tabs, trusted pairs and human acceptance remain open. | Retain source/export/layer/order/offset and fourteen-cell/actual-PNG regressions; finish the representative class/gender/equipment matrix and final-source same-state same-EXE screenshots, then human acceptance. |
| WN-CHAR-002 | P1 | Equipment interaction and item-operation UI | Crystal item-cell double-click/drag behavior, exact destination choice, lock/receipt lifecycle and equip/unequip state changes work without a substitute operation panel. | The old unequip command incorrectly targeted `equipment/-1`. The bounded bag path now selects the first free normalized bag index accepted by the current Gateway and waits for authoritative state; full-bag/expansion/stale-cell regressions pass. The generic `Item / Use / Equip / Unequip` popup and name-based equip-slot guessing are still non-source. Belt-first amulet removal/merge cannot yet be expressed by the normalized-bag endpoint and remains explicitly open, not falsely routed to a bag cell. | Same-process real-input equip/unequip cycle plus correct first-free-cell evidence; source double-click/drag/lock/ack and paired-slot selection matrix; explicit raw-Crystal versus normalized-Gateway slot contract; belt/amulet and full-capacity tests; human comparison. |
...
- The older unrelated dirty `overlays.rs` inventory-expansion note is
  historical. The item/actor asset round started from clean
  `0e10b08bc0865831d57839055415674be6ca0a65` and does not edit that file.
  Its bounded evidence must not be counted as full expansion/item parity.
