...
2. 进游戏后右下角出现 **On-chain Mine (testnet)** 面板；矿脉 (335,270) 应显示 `full vein 满`
   （进图即播种）。
3. **挖矿 = 走到矿脉前对它发起攻击**（点矿脉格 (335,270)，或按方向键朝它走一步：离得远会自动
   走过去，到位即开挖；就是原版"走到矿区用武器敲矿脉"的手感）。**连接钱包后开挖会自动连挖**
   （`NEXT_PUBLIC_ONCHAIN_MINE_SWING_INTERVAL_MS`，默认 900ms/下），直到你走开或矿脉挖空；
   未连钱包时点一下只挥一镐（避免无人值守时空攒）。攒满 5 下 → 自动弹钱包签名 **1 笔
