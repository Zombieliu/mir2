...
Active backend/global round: R226
...
Backend/server parity estimate: **99.70%**. This is not the whole-project score; it only covers Rust backend, gateway, protocol, persistence, and server gameplay behavior.
...
| --- | ---: | ---: | ---: | --- |
| Backend/server behavior parity | 45% | 99.70% | 44.9% | Current backend tracker is green through generated `RandomItemStats.ini` profile import, generated Crystal `GROUP` drop tree semantics, source-confirmed immediate drop visibility with pickup-only owner windows, current-cell pickup scan/skip edge cases, slot/stack-only pickup and harvest gain checks, Crystal `HarvestMonster` pending drop generation/next-call transfer/full-bag retry semantics, harvest no-drop/full-bag localization, harvest success-chat cleanup, harvest owner/EXPOwner corpse scan rejection, ordinary/debug map-transfer success-chat cleanup, unknown map-transfer localization, Crystal `SellItem` item flag/type/price/gold-cap rejection edges, Crystal `StoreItem` / `TakeBackItem` storage-page/item-flag/capacity/ack semantics including rental `DontStore`, storage NPC object/range validation across current storage-family item actions, live-NPC/range validation across current buy/sell/repair item-service actions, active-service/range/format/reset parity across current storage password flows, successful current storage open/unlock `UserStorage` follow-up packets, repeated unchanged storage-open `UserStorage` resend suppression, full-backing-length `UserStorage` payload parity when expanded storage is inactive, expired expanded-storage downgrade parity across current `StartGame` and the first world tick, expanded-storage expiry localization, current `EquipItem(grid=Storage)` plus exact-slot `RemoveItem(grid=Inventory|Storage)` parity, current `RemoveSlotItem` source-grid / parent-equipment fallback guard parity, current `MysteryWater` unlock-cursed flow plus cursed current `EquipItem` / `RemoveItem` and storage replacement `DontStore` guard parity, real `NeedIdentify` / `SoulBoundId` metadata parity across current item/equipment round-trips and equip rejection, current-data template-backed `UseItem` routing for HP/MP consumables, duration buffs, town teleports, repair oils, equipment slot mapping, `NoEscape` / `NoRandom` / `NoDrug` map-rule rejection on escape/random/potion uses, right-side explicit equip compatibility for manifest-backed ring/bracelet/amulet items, explicit `EquipItem` requirement gating, and chat-free successful modeled use-equip, non-inventory equipment-use rejection, unusable item fallback, missing-item use failure, static HP/MP potion timed recovery, static repair-oil success/failure chat surfaces, static benediction-oil invalid failure, static town-teleport success, static repair-powder success/failure, successful DropItem, successful SplitItem, normal pickup success, owner-blocked pickup localization, pickup full-bag localization, Stage 5 item socket/seal rejection-key localization, Stage 5 item-seal reseal-delay localization, Stage 5 item socket/seal success-chat localization, Stage 5 socket/seal invalid-source localization, Stage 5 socket/seal missing-source localization, Stage 5 socket/seal missing-equipped-item localization, Stage 5 socket metadata-missing localization, Stage 5 missing-entity localization, Stage 5 event-spawn missing-template/player-position localization, Stage 5 missing-hero localization, Stage 5 invalid trade/mail/auction/unknown-command input localization, Stage 5 inactive-trade localization, Stage 5 gold-shop purchase localization, Stage 5 full-bag economy/helper rejection localization, Stage 5 low-gold rejection localization, Stage 5 trade-success localization, Stage 5 credit-shop purchase and insufficient-credit localization, Stage 5 craft no-ore localization, Stage 5 guild creation localization, Stage 5 generic helper success-chat cleanup including event/hero helper successes, hero-inventory use failed-ack behavior, missing-source DropItem failed-ack behavior, and high-level drop helper missing-item chat cleanup, high-level use/drop before-start chat cleanup for dynamic manifest-backed equipment, plus credit-shop mailbox delivery, silent `BuyItem` rejection edges, NPC repair/sell/storage item-service parity, current Jev random-stat payloads, item packet surfaces, combine/merge/split/drop/move guards, quest-drop `Q`, drop placement, pickup, and AddItem belt-priority work; combat, map events, and several economy edge cases remain. |
| Frontend/client UI and interaction parity | 25% | 88.0% | 22.0% | Web client builds and smoke tests pass, map/UI pipeline exists, current gameplay panels are usable, and Stage 5 UI smoke now covers login/select lifecycle, compact layout/text/multi-panel bounds, minimap, belt, chat/channel filters, storage, storage password panel entry/mismatch/no-service submit, system menu QA/transfer routes, Mail/Report/NPC panels with link-capable rendering, character repair/special-repair/remove/spell-cast flows, HUD buttons, inventory use/equip/gold/move/split/drop/sell, ground item/gold pickup, Store Item no-service preservation, and Take Back no-service preservation evidence; exact Crystal visual layout, animation timing, sound/effects, service-backed economy/storage flows, and pixel-level UI parity are still incomplete. |
...
| End-to-end integration and live parity harness | 10% | 79% | 7.9% | Local packet trace, load, smoke, gateway harnesses, route screenshots, login/select lifecycle, map API/minimap JSON evidence, ground pickup, menu transfer, spell-cast, and broad Stage 5 systems manifest evidence exist; accepted live Crystal side-by-side traces for the full representative matrix are still missing. |
| Playability, operations, and hardening | 5% | 88% | 4.4% | Save/reload, reconnect, load harnesses, smoke route stability, broad-system baselines, compact multi-panel layout checks, and refreshed WS load 64/64 evidence exist; long-duration production acceptance, telemetry/rollback readiness, and late-game multi-system playability remain open. |
| **Total** | **100%** |  | **90.0%** | Whole-project estimate, not interchangeable with the backend 99.70% tracker. |
