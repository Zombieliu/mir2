...
- **Monster slow / red / stun soft-modifier receipt** — a *holding* poison (Frozen/Stun/Paralysis/LR-Paralysis) now stops a monster outright (item 16), but the soft modifiers remain: Slow's `MoveSpeed`/`AttackSpeed` reduction is a sub-second delta the 1-second tick cannot express, and Red's `ArmourRate -= 0.5` / Stun's `DamageRate += 0.5` have no hook because the runtime does not yet mitigate player→monster damage with monster armour (monsters take attack damage raw).
