...
- **Magic spells use AC, not MAC** — the player→monster hit/armour resolution is uniformly AC-typed (ACAgility): the accuracy/agility hit roll and the armour subtraction both read physical AC. Crystal resolves a Wizard/Taoist magic hit against `MAC`/`MACAgility` (magic armour + a MagicResist miss roll). Splitting the resolution per-skill (physical → AC, magic → MAC) is a paired refinement of both the hit roll and the armour roll, deferred so the two stay consistent.
