...
运行期间 Zone CPU 饱和而内存仍有余量，因此该结果是当前 10 CPU Docker 主机
的 CPU 容量边界，不是可以通过增加内存解决的问题。`10 CPU / 7.5GiB` 只是
允许采集真实证据的最低运行配置，不代表必然通过。4 CPU / 8GiB 的 Early
配置仍只承诺约 100 CCU；1,000 CCU 的正式通过需要继续优化热路径或换更高
CPU 配额的验收机，不能通过放宽 p95 或负载覆盖率获得。
