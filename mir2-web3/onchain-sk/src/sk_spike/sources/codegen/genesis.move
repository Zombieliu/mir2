...
module sk_spike::genesis {
    ...
    public fun run(dapp_hub: &mut DappHub, clock: &Clock, ctx: &mut TxContext) {
        ...
        let mut ds = dapp_system::create_dapp(dapp_key, dapp_hub, string(b"sk_spike"), string(b"SK0 session-key spike: a per-user counter to exercise session writes"), 1, clock, ctx);

        // Set up initial DApp state (e.g. default resource values).
        sk_spike::deploy_hook::run(&mut ds, ctx);
