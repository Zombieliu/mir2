...
impl SocialModel {
    ...
    pub fn apply_packet(&mut self, packet: &str, payload: &Value) -> bool {
        ...
        match packet {
            "SwitchGroup" => {
                let Some(allow) = payload.get("allowGroup").and_then(Value::as_bool) else { return false; };
                self.group.allow_invites = allow;
                changed = true;
            }
            "DeleteGroup" => { self.group = GroupModel::default(); changed = true; }
            "DeleteMember" => {
                let Some(name) = clean_name(payload.get("name")) else { return false; };
            ...
            "GroupMemberInfo" => {
                let Some(members) = payload.get("members").and_then(Value::as_array) else { return false; };
                if members.len() > MAX_GROUP_MEMBERS { return false; }
                let parsed = members.iter().filter_map(parse_group_member).collect::<Vec<_>>();
                if parsed.len() != members.len() { return false; }
            ...
            "GuildStatus" => {
                let Some(raw_name) = guild_field(payload, "guild_name", "guildName").and_then(Value::as_str) else { return false; };
                let Some(raw_rank) = guild_field(payload, "guild_rank_name", "guildRankName").and_then(Value::as_str) else { return false; };
                if raw_name.trim().is_empty() || raw_rank.trim().is_empty() {
                    self.guild = GuildModel::default();
                    success = Some(true);
                    self.last_event = Some(SocialAuthoritativeEvent { packet: packet.to_owned(), success, subject: None, from: None, to: None });
                    self.normalize();
                    return true;
                }
                let Some(name) = clean_name(Some(&Value::String(raw_name.to_owned()))) else { return false; };
                ...
                self.guild.experience = value_i64(guild_field(payload, "experience", "experience")).unwrap_or(0).max(0);
                self.guild.max_experience = value_i64(guild_field(payload, "max_experience", "maxExperience")).unwrap_or(0).max(0);
                self.guild.gold = value_u32(guild_field(payload, "gold", "gold")).unwrap_or(0);
                self.guild.member_count = value_u16(guild_field(payload, "member_count", "memberCount")).unwrap_or(0);
                self.guild.max_members = value_u16(guild_field(payload, "max_members", "maxMembers")).unwrap_or(0);
                self.guild.my_options = value_u8(guild_field(payload, "my_options", "myOptions")).unwrap_or(0);
                self.guild.my_rank_id = value_i32(guild_field(payload, "my_rank_id", "myRankId")).unwrap_or(-1);
                changed = true;
            }
            "GuildNoticeChange" => {
                let Some(notice) = bounded_strings(payload.get("notice"), MAX_NOTICE_LINES) else { return false; };
                self.guild.notice = notice;
                changed = true;
            }
            "GuildMemberChange" => {
                let Some(ranks) = payload.get("ranks").and_then(Value::as_array) else { return false; };
                if ranks.len() > MAX_GUILD_RANKS { return false; }
                let mut all = Vec::new();
                let mut parsed_ranks = Vec::new();
                for rank in ranks {
                    let Some(name) = clean_name(rank.get("name")) else { return false; };
                    let members = rank.get("members").and_then(Value::as_array).map_or(&[][..], Vec::as_slice);
                    if all.len() + members.len() > MAX_GUILD_MEMBERS { return false; }
                    let mut parsed_members = Vec::new();
                    for member in members {
                        let Some(mut parsed) = parse_guild_member(member) else { return false; };
                        parsed.rank_name = Some(name.clone());
                        parsed.rank_index = value_u8(rank.get("index"));
                        all.push(parsed.clone());
                        parsed_members.push(parsed);
                    }
                    parsed_ranks.push(GuildRankModel { name, index: value_i32(rank.get("index")).unwrap_or(0), options: value_u8(rank.get("options")).unwrap_or(0), members: parsed_members });
                }
            ...
            "TradeRequest" => {
                let Some(name) = clean_name(payload.get("partnerName").or_else(|| payload.get("name"))) else { return false; };
            ...
            "TradeGold" => {
                let Some(amount) = value_u32(payload.get("partnerGold").or_else(|| payload.get("amount"))) else { return false; };
                self.trade.partner_gold = amount;
                changed = true;
            }
            "TradeItem" => {
                let items = payload.get("partnerItems").or_else(|| payload.get("tradeItems")).and_then(Value::as_array);
                let Some(items) = items else { return false; };
                if items.len() > MAX_TRADE_ITEMS { return false; }
                let parsed = items.iter().filter_map(parse_trade_item).collect::<Vec<_>>();
                if parsed.len() != items.iter().filter(|item| !item.is_null()).count() { return false; }
                self.trade.partner_items = parsed;
                changed = true;
            }
            "TradeConfirm" => { self.trade.partner_confirmed = true; changed = true; }
            "TradeCancel" => { self.trade = TradeModel::default(); changed = true; }
        ...
        if changed {
            ...
            self.last_event = Some(SocialAuthoritativeEvent { packet: packet.to_owned(), success, subject, from, to });
            self.normalize();
            true
        } else { false }
    }
    ...
    fn reconcile_pending(&mut self, old: &SocialModel) {
        ...
        self.pending.retain(|operation| !Self::proven_complete(operation, event, old, &current));
    }
    ...
    fn proven_complete(
        ...
        match operation {
            SocialPendingOperation::GroupSwitch { allow_group } => {
                event.is_some_and(|e| e.packet == "SwitchGroup") && current.group.allow_invites == *allow_group
            }
            SocialPendingOperation::GroupAdd { name } => {
                event.is_some_and(|e| {
                    (e.packet == "AddMember" && e.subject.as_deref() == Some(name.as_str()))
                        || (e.packet == "GroupMemberInfo" && current.group.members.iter().any(|member| member.name == *name))
                })
            }
            SocialPendingOperation::GroupRemove { name } => {
                event.is_some_and(|e| {
            ...
            SocialPendingOperation::GroupInviteAccept => {
                // Crystal's ordinary AddMember/GroupMemberInfo packets do
                // not identify the accepted invite or carry a request id.
                // Treating either packet as an ACK can confirm an unrelated
                // group update, so keep this pending until a future explicit
                // receipt-capable protocol is available.
                false
            }
            SocialPendingOperation::GuildInfo => event.is_some_and(|e| matches!(e.packet.as_str(), "GuildStatus" | "GuildMemberChange")),
            ...
            SocialPendingOperation::GuildNotice => event.is_some_and(|e| e.packet == "GuildNoticeChange"),
            // GuildStatus is a snapshot refresh, not proof that this invite
            // was accepted. There is no correlatable Crystal ACK here.
            SocialPendingOperation::GuildInviteAccept => false,
            SocialPendingOperation::TradeRequest | SocialPendingOperation::TradeReply => {
                event.is_some_and(|e| matches!(e.packet.as_str(), "TradeRequest" | "TradeAccept")) && old.trade.state != current.trade.state
            }
            ...
            SocialPendingOperation::TradeGold { .. } | SocialPendingOperation::TradeConfirm { .. } => false,
...
fn parse_group_member(value: &Value) -> Option<GroupMemberModel> {
    ...
    Some(GroupMemberModel { name, leader: value.get("leader").and_then(Value::as_bool).unwrap_or(false), online: value.get("online").and_then(Value::as_bool).unwrap_or(false), level: value_u16(value.get("level")), class: value_u8(value.get("class")), hp: value_i32(value.get("hp")), max_hp: value_i32(value.get("maxHp")), map: clean_name(value.get("map")) })
}

fn parse_guild_member(value: &Value) -> Option<GuildMemberModel> {
    Some(GuildMemberModel { name: clean_name(value.get("name"))?, id: value_i32(value.get("id")).unwrap_or(0), online: value.get("online").and_then(Value::as_bool).unwrap_or(false), ..Default::default() })
}

fn parse_trade_item(value: &Value) -> Option<TradeItemModel> {
    if value.is_null() { return None; }
    Some(TradeItemModel { unique_id: value_u64(value.get("uniqueId")), item_index: value_i32(value.get("itemIndex")), name: clean_name(value.get("name")), count: value_u16(value.get("count")).unwrap_or(1).max(1) })
}

fn upsert_group_member(group: &mut GroupModel, name: String) {
    if let Some(member) = group.members.iter_mut().find(|member| member.name == name) { member.online = true; } else if group.members.len() < MAX_GROUP_MEMBERS {
        group.members.push(GroupMemberModel { name: name.clone(), online: true, leader: group.members.is_empty(), ..Default::default() });
    }
    group.active = !group.members.is_empty();
    if group.leader_name.is_none() { group.leader_name = Some(name); }
}

fn value_u8(value: Option<&Value>) -> Option<u8> { value?.as_u64().and_then(|n| u8::try_from(n).ok()) }
fn value_u16(value: Option<&Value>) -> Option<u16> { value?.as_u64().and_then(|n| u16::try_from(n).ok()) }
fn value_u32(value: Option<&Value>) -> Option<u32> { value?.as_u64().and_then(|n| u32::try_from(n).ok()) }
fn value_u64(value: Option<&Value>) -> Option<u64> { value?.as_u64() }
fn value_i32(value: Option<&Value>) -> Option<i32> { value?.as_i64().and_then(|n| i32::try_from(n).ok()) }
fn value_i64(value: Option<&Value>) -> Option<i64> { value?.as_i64() }
...
mod tests {
    ...
    fn malformed_and_oversized_social_packets_fail_closed() {
        ...
        assert!(!model.apply_packet("GroupMemberInfo", &json!({"members": (0..16).map(|i| json!({"name": i.to_string()})).collect::<Vec<_>>()})));
    ...
    fn guild_packets_accept_server_packet_snake_case_and_camel_case_compatibility() {
        ...
        let member_payload = member_change.get("GuildMemberChange").expect("variant payload");
        assert!(model.apply_packet("GuildMemberChange", member_payload));
        assert_eq!(model.last_event.as_ref().and_then(|event| event.subject.as_deref()), Some("Miner"));
        assert_eq!(model.guild.members[0].name, "Miner");

        let mut snake = SocialModel::default();
        assert!(snake.apply_packet("GuildStatus", &json!({
        ...
        let invite = serde_json::to_value(ServerPacket::GuildInvite { name: "Invitee".into() })
            .expect("GuildInvite serializes");
        assert!(model.apply_packet("GuildInvite", invite.get("GuildInvite").expect("variant payload")));

        let mut camel = SocialModel::default();
        assert!(camel.apply_packet("GuildStatus", &json!({
    ...
    fn reconcile_packet(model: &mut SocialModel, packet: &str, payload: Value) {
        assert!(model.apply_packet(packet, &payload));
    ...
    fn every_authoritative_group_and_guild_operation_releases_only_on_matching_state() {
        ...
        remove.group.members = vec![GroupMemberModel { name: "Bob".into(), ..Default::default() }];
        ...
        group_invite.group.pending_invite_from = Some("Leader".into());
        ...
        assert_eq!(group_invite.pending.len(), 1, "ordinary AddMember is not an invite ACK");
        reconcile_packet(&mut group_invite, "GroupMemberInfo", json!({"members":[{"name":"Me"}]}));
        assert_eq!(group_invite.pending.len(), 1, "ordinary group snapshot is not an invite ACK");

        let mut guild_info = SocialModel::default();
        guild_info.begin_pending(SocialPendingOperation::GuildInfo);
        reconcile_packet(&mut guild_info, "GuildStatus", json!({"guildName":"Guild","guildRankName":"Chief"}));
        assert!(guild_info.pending.is_empty());

        let mut member = SocialModel::default();
        member.begin_pending(SocialPendingOperation::GuildMember { name: "Miner".into() });
        reconcile_packet(&mut member, "GuildMemberChange", json!({"name":"Other","ranks":[]}));
        assert_eq!(member.pending.len(), 1);
        reconcile_packet(&mut member, "GuildMemberChange", json!({"name":"Miner","ranks":[]}));
        assert!(member.pending.is_empty());

        let mut notice = SocialModel::default();
        notice.begin_pending(SocialPendingOperation::GuildNotice);
        reconcile_packet(&mut notice, "GuildNoticeChange", json!({"notice":["Hello"]}));
        ...
        reconcile_packet(&mut invite, "GuildStatus", json!({"guildName":"Guild","guildRankName":"Member"}));
        assert_eq!(invite.pending.len(), 1, "ordinary GuildStatus is not an invite ACK");
    }
    ...
    fn failed_or_unrelated_trade_packets_do_not_fake_completion() {
        ...
        reconcile_packet(&mut deposit, "DepositTradeItem", json!({"from":2,"to":0,"success":false}));
        assert_eq!(deposit.pending.len(), 1);
        reconcile_packet(&mut deposit, "DepositTradeItem", json!({"from":2,"to":0,"success":true}));
    ...
    fn trade_deposit_and_retrieve_ack_match_packet_and_both_slots() {
        ...
        reconcile_packet(&mut model, "DepositTradeItem", json!({"from":0,"to":3,"success":true}));
        assert_eq!(model.pending.len(), 2, "wrong slots must release neither operation");
        reconcile_packet(&mut model, "RetrieveTradeItem", json!({"from":2,"to":0,"success":true}));
        assert_eq!(model.pending.len(), 2, "wrong packet and slots must release neither operation");
        reconcile_packet(&mut model, "DepositTradeItem", json!({"from":2,"to":0,"success":true}));
        assert_eq!(model.pending, vec![SocialPendingOperation::TradeRetrieve { from: 0, to: 3 }]);
        reconcile_packet(&mut model, "RetrieveTradeItem", json!({"from":0,"to":3,"success":true}));
        assert!(model.pending.is_empty());
    }
    ...
    fn out_of_order_snapshot_does_not_release_unacknowledged_trade_or_group_work() {
        ...
        older.group.members.push(GroupMemberModel { name: "Bob".into(), ..Default::default() });
        model.apply_authoritative(older);
        assert_eq!(model.pending.len(), 2);

        assert!(model.apply_packet("DepositTradeItem", &json!({"from": 2, "to": 0, "success": false})));
        model.apply_authoritative(model.clone());
        assert_eq!(model.pending.len(), 2);
        assert!(model.apply_packet("DepositTradeItem", &json!({"from": 2, "to": 0, "success": true})));
        model.apply_authoritative(model.clone());
        assert_eq!(model.pending.len(), 1);
    }
}
