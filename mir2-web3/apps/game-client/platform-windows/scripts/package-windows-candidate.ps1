...
if ($SelfTest) {
    ...
    try {
        ...
        $allowedLayoutVectors += 'mir2-assets/original-ui/Sound/105.wav'
...
try {
 ...
 foreach ($index in @(197,205,206,207,360,361,362,366,367,368,431,1340,1350) + @(450..468) + @(470..475)) { $requiredSources += (Join-Path $PublicRoot "original-ui\Prguse2\$index.png") }
...
$requiredSourceTrees = @((Join-Path $PublicRoot 'bevy-entity-atlases'), (Join-Path $PublicRoot 'generated\map-atlas'), $nativeKeyedMapRoot, $MapPackRoot, (Join-Path $PublicRoot 'original-effects'), (Join-Path $PublicRoot 'original-ui\ChrSel'), (Join-Path $PublicRoot 'original-ui\Cursors'), (Join-Path $PublicRoot 'original-ui\Help'), (Join-Path $PublicRoot 'original-ui\Items'), (Join-Path $PublicRoot 'original-ui\MMap'), (Join-Path $PublicRoot 'original-ui\Prguse'), (Join-Path $PublicRoot 'original-ui\Prguse2'), (Join-Path $PublicRoot 'original-ui\StateItem'), (Join-Path $PublicRoot 'original-ui\UI_32bit'), (Join-Path $PublicRoot 'original-ui\Title'), (Join-Path $PublicRoot 'original-ui\Monster\000'), (Join-Path $PublicRoot 'original-ui\NPC\00'))
...
foreach ($soundName in @('Login2.wav','Select2.wav','103.wav','M8-1.wav','M31-0.wav','M31-1.wav','M31-2.wav','M34-0.wav','M34-1.wav','M34-2.wav','M39-0.wav','M39-1.wav','M40-0.wav','M61-0.wav','M61-1.wav','M64-0.wav','M64-1.wav','M64-2.wav','M79-1.wav') + $playerCombatSoundNames + $monsterCombatSoundNames + $frostCrunchSoundNames + $hallucinationSoundNames) { $sound = Join-Path $PublicRoot ('original-ui\Sound\' + $soundName); Assert-NoReparseTree -Path $sound; Assert-NoAlternateDataStreams -Path $sound }
 $buttonCSound = Join-Path $PublicRoot 'original-ui\Sound\105.wav'; Assert-NoReparseTree -Path $buttonCSound; Assert-NoAlternateDataStreams -Path $buttonCSound
...
try {
    ...
    Copy-FileDefaultDataOnly -Source (Join-Path $PublicRoot 'original-ui\Sound\105.wav') -Destination (Join-Path $soundDest '105.wav')
