...
function Test-PackageRelativeFileAllowed {
    ...
    if ($RelativePath -ceq 'mir2-assets/original-ui/Sound/Login2.wav' -or $RelativePath -ceq 'mir2-assets/original-ui/Sound/Select2.wav' -or $RelativePath -ceq 'mir2-assets/original-ui/Sound/M40-0.wav') { return $true }
...
if ($SelfTest) {
    ...
    try {
        ...
        $allowedLayoutVectors = @('mir2-platform-windows.exe','mir2-client.toml','README-START.txt','CONTROLS.txt','KNOWN-ISSUES.md','BUILD-ATTESTATION.json','PACKAGE-MANIFEST.json','VERSION.json','RELEASE-STATEMENT.json','RELEASE-STATEMENT.p7s','mir2-assets/bevy-entity-atlases/manifest.json','mir2-assets/generated/map-atlas/0.png','mir2-assets/generated/native-map-keyed/manifest.json','mir2-assets/crystal-map-pack/0.map.gz','mir2-assets/original-effects/effects.generated.json','mir2-assets/original-effects/sprite.v1.final.PNG',('mir2-assets/original-effects/folder'+$unicodeNoBreakSpace+'/0.png'),('mir2-assets/original-effects/payload'+$unicodeFullwidthFullStop+'exe.png'),'mir2-assets/original-ui/frame-sets.generated.json','mir2-assets/original-ui/ChrSel/0.png','mir2-assets/original-ui/Sound/Login2.wav','mir2-assets/original-ui/Sound/Select2.wav','mir2-assets/original-ui/Sound/M40-0.wav')
...
$lightningSound = Join-Path $PublicRoot 'original-ui\Sound\M40-0.wav'
$lightningSoundItem = Get-Item -LiteralPath $lightningSound
if ([int64]$lightningSoundItem.Length -ne 247772 -or (Get-FileHash -LiteralPath $lightningSound -Algorithm SHA256).Hash.ToUpperInvariant() -cne '05E08C3AA3ADF166A3FDF9462279024898217F4F936BBD28A1FB6EA75BF92A4E') { throw 'Lightning M40-0.wav source identity mismatch' }
...
foreach ($sound in @((Join-Path $PublicRoot 'original-ui\Sound\Login2.wav'), (Join-Path $PublicRoot 'original-ui\Sound\Select2.wav'), (Join-Path $PublicRoot 'original-ui\Sound\M40-0.wav'))) { Assert-NoReparseTree -Path $sound; Assert-NoAlternateDataStreams -Path $sound }
...
try {
    ...
    foreach ($sound in @('Login2.wav', 'Select2.wav', 'M40-0.wav')) { Copy-FileDefaultDataOnly -Source (Join-Path $PublicRoot ('original-ui\Sound\' + $sound)) -Destination (Join-Path $soundDest $sound) }
