...
$MonsterCombatSoundNames = @('005-3.wav')
...
function Test-PackageRelativeFileAllowed {
    ...
    if($RelativePath -ceq 'mir2-assets/original-ui/Sound/005-3.wav'){return $true}
...
if ($SelfTest) {
    ...
    try {
        ...
        $allowedLayoutVectors += 'mir2-assets/original-ui/Sound/005-3.wav'; foreach($rel in $allowedLayoutVectors){if(-not(Test-PackageRelativeFileAllowed -RelativePath $rel -ExeName 'mir2-platform-windows.exe')){throw "strict allowlist rejected valid VIS-04 path: $rel"}}
