...
function Get-RequiredCandidateFiles {
    ...
    $required = @($ExeName, 'BUILD-ATTESTATION.json', 'PACKAGE-MANIFEST.json', 'VERSION.json', 'RELEASE-STATEMENT.json', 'RELEASE-STATEMENT.p7s', 'mir2-client.toml', 'README-START.txt', 'CONTROLS.txt', 'KNOWN-ISSUES.md', 'mir2-assets\bevy-entity-atlases\manifest.json', 'mir2-assets\generated\map-atlas\manifest.json', 'mir2-assets\generated\native-map-keyed\manifest.json', 'mir2-assets\original-effects\effects.generated.json', 'mir2-assets\crystal-map-pack\0.map.gz', 'mir2-assets\original-ui\frame-sets.generated.json', 'mir2-assets\original-ui\Prguse\20.png', 'mir2-assets\original-ui\Sound\Login2.wav', 'mir2-assets\original-ui\Sound\Select2.wav', 'mir2-assets\original-ui\Sound\M40-0.wav')
...
if ($SelfTest) {
    ...
    try {
        ...
        $unicodeNoBreakSpace=[string][char]0x00A0;$unicodeFullwidthFullStop=[string][char]0xFF0E;$allowedLayoutVectors=@('mir2-platform-windows.exe','mir2-client.toml','README-START.txt','CONTROLS.txt','KNOWN-ISSUES.md','BUILD-ATTESTATION.json','PACKAGE-MANIFEST.json','VERSION.json','RELEASE-STATEMENT.json','RELEASE-STATEMENT.p7s','mir2-assets/bevy-entity-atlases/manifest.json','mir2-assets/generated/map-atlas/0.png','mir2-assets/generated/native-map-keyed/manifest.json','mir2-assets/crystal-map-pack/0.map.gz','mir2-assets/original-effects/effects.generated.json','mir2-assets/original-effects/sprite.v1.final.PNG',('mir2-assets/original-effects/folder'+$unicodeNoBreakSpace+'/0.png'),('mir2-assets/original-effects/payload'+$unicodeFullwidthFullStop+'exe.png'),'mir2-assets/original-ui/frame-sets.generated.json','mir2-assets/original-ui/ChrSel/0.png','mir2-assets/original-ui/Sound/Login2.wav','mir2-assets/original-ui/Sound/Select2.wav','mir2-assets/original-ui/Sound/M40-0.wav');foreach($rel in $allowedLayoutVectors){if(-not(Test-PackageRelativeFileAllowed -RelativePath $rel -ExeName 'mir2-platform-windows.exe')){throw "strict allowlist rejected valid path: $rel"}}
...
$lightningSound = Join-Path $PackageRoot 'mir2-assets\original-ui\Sound\M40-0.wav'
if (Test-Path -LiteralPath $lightningSound -PathType Leaf) { $lightningSoundItem = Get-Item -LiteralPath $lightningSound; if ([int64]$lightningSoundItem.Length -ne 247772 -or (Get-FileHash -LiteralPath $lightningSound -Algorithm SHA256).Hash.ToUpperInvariant() -cne '05E08C3AA3ADF166A3FDF9462279024898217F4F936BBD28A1FB6EA75BF92A4E') { Fail 'Lightning M40-0.wav identity mismatch' } }
try {
