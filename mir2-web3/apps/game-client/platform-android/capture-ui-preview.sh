...
task_scenes="login empty-roster roster create password safekey delete-character connecting starting disconnected hud world-render inventory inventory-amount character skills quests options platform menu gameshop npcshop mail mail-compose bigmap storage group guild trade chat-settings npc death chat help"
...
echo "$(printf '%s\n' $task_scenes | wc -l | tr -d ' ') offline specimens captured; NOT live, physical-device or full interaction acceptance."
