...
export async function POST(request: Request) {
  ...
  if (!base) {
    return Response.json(
      { error: "channel identity Gateway proxy is not configured" },
      { status: 503 },
    );
  }
  try {
    const upstream = await fetch(`${base}/v1/channels/identity/link`, {
      method: "POST",
      headers: {
    ...
    return Response.json({ error: "channel identity Gateway is unavailable" }, { status: 502 });
  }
}
