...
## 2026-08-18 — Quest window alignment and drag closure

- A final 3840x2160 browser gate found that the newly stage-anchored Account Security trigger still intercepted the top-right language selector and would share space with the in-game minimap. The trigger now uses the stage top-left, with a responsive-stage regression locking that non-overlapping anchor before publication.
- Final visual review also found the tutorial coach card in the browser letterbox. Tutorial, debug, and death/revive overlays now share the stage portal, with spotlight geometry normalized for scaled stages.
- The resource-cache progress panel was still fixed to the browser's bottom-left at 4K. It now portals into the game stage and uses stage-relative absolute sizing, keeping the loading percentage visible next to the playfield.
- The same live quest diary exposed q22 `Forest Yeti's Threat` as the final Chinese-mode English fallback. Added all four stage presentations plus Forest Yeti and Precision Pendant display names, and extended the localization regression matrix to q22.
- The standalone quest diary reused both the Crystal mail frame and its separate `MAIL` title sprite, so the localized `任务` label rendered over `MAIL` as `任务IL`. The frame's baked-in mail grid also remained visible beneath independently positioned quest tabs/list/detail controls, making otherwise close coordinates look visibly misaligned.
- Removed the obsolete mail title sprite, masked only the frame's stale interior grid, and normalized tabs, list, detail, and action areas onto one 292px content baseline. Tab height/list start are now explicit, so the first quest row no longer overlaps the two-line filter tabs.
- Added a real title-bar pointer drag path with pointer capture, stage-scale correction, full-window boundary clamping, resize re-clamping, and optional persisted placement. The safe first position is centered at stage coordinates `(356,40)`, clear of the top-right account-security control.
- Live acceptance used the already logged-in local character without reading or entering credentials. A real Chrome pointer drag moved the window from `(356,40)` to `(176,120)`; a second drag beyond the viewport clamped it to `(4,4)` with the entire 312x444 window still inside the stage; the panel was then restored to `(356,40)`. The localized title was exactly `任务`, no `/Title/7.png` remained inside the window, and the tab/detail edges both measured `[421.5,713.5]` in the served page.
- Passed `test:system-menu-quest-button`, Web TypeScript, `next build --webpack`, `git diff --check`, and the required generic web-game client smoke. Its clean isolated browser reported `screen=login` / `sceneInteractionReady=true`; the existing logged-in Chrome tab completed the in-world visual/interaction acceptance with zero app-origin console errors (remaining warnings were browser-extension-origin only).

## 2026-08-18 — Local UI/i18n entity-runtime closure

- The isolated `codex/fix-ui-i18n-parity` worktree first lacked generated `public/bevy-runtime` artifacts, so the versioned WebGPU module returned 404 and the client stayed on the compatibility renderer. Fetched and hash-verified all four pinned `bevy-9a5cbecc8f85ff75` WebGPU/WebGL2 JS/WASM artifacts and verified their versioned URLs, MIME types, and the seven-page starter entity atlas.
- A second independent bug kept NPCs/monsters invisible after Bevy booted: without `NEXT_PUBLIC_MIR2_ASSET_BASE_URL`, `original-scene-sprite-meta.ts` discarded the complete source-library index before probing same-origin assets. The checked-in `Monster/004`, `Monster/005`, `NPC/00`, and other metadata returned HTTP 200 but were incorrectly classified as nonexistent, leaving 28 entities with only the player's three render layers.
- Source-index libraries are now recognized in local/offline mode; an unavailable source library still performs the existing bounded fetch/failure path and is memoized as missing. The regression test explicitly instantiates the module with no remote asset base and verifies Monster keys remain available while Map keys remain excluded, and `test:scene-sprite-meta` is now part of the frontend logic gate.
- Rebuilt and restarted the production-shaped local web app, entered the existing local test character through the already-filled form without reading or entering credentials, and captured an in-world browser render. Bevy reported 28 entities, 30 layers, 17 loaded sprite libraries, seven packed atlases, and zero failed static scene assets; deer, hens, guards, scarecrows, and NPC bodies were visibly rendered. App-origin console errors were zero (remaining warnings were extension-origin only).
- Passed `test:scene-sprite-meta`, `test:player-frames`, `npm run typecheck`, `next build --webpack`, whitespace checks, and the generic web-game client login/scene-readiness smoke.
...
- CI follow-up: the cross-platform developer-environment workflow now fetches the pinned runtime before validation, and the developer release gate requires all four runtime files to be ignored/untracked while retaining exact SHA-256, WASM header, manifest-version, and clean-worktree checks. The local clean-runtime simulation and complete developer-release gate passed with the pinned Crystal submodule.
- Compose follow-up: the first clean-runner smoke proved Gateway health, Player Web, revision, and Starter assets, then exposed a 404 for the externalized Bevy WASM because that independent job had not populated the runtime. Developer Web startup now fetches and verifies the pinned runtime before Next starts, Bash/PowerShell `verify` do the same before release validation, and the release checker locks both startup paths against regression.
- Linux permission follow-up: the next clean runner exposed the expected bind-mount UID boundary when the Web container tried to create the downloader's sibling transaction directory. The Bash launcher now performs the atomic fetch through an entrypoint-free one-shot container using the host UID/GID before `up` or `verify`; PowerShell uses the image's unprivileged `node` identity for Docker Desktop. Web startup still re-verifies the four hashes before serving, so no writable-public-directory relaxation or unchecked fallback was introduced.
