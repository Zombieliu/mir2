...
2026-07-06 movement pipeline probe/acceptance follow-up:

- Built out the `mir2-probe` movement diagnostics so frame longtasks, recent resources, movement sent/received packets, gateway tick health, and suite summaries are captured by `apps/web/scripts/run-probe.mjs`.
- Fixed the client-side hot path that was re-acquiring the same prebuilt Bevy entity atlas on animation-source key changes; the final walk-right repro no longer shows action-window longtasks or `starter-bichon-base.png` decode/readback during movement.
- Tightened client movement dispatch: render prediction may still lead up to two tiles, but command pipelining now only uses a one-tile lead and waits on opposing outstanding movement before reversing direction. The movement confirmation loop also sends bounded gateway ticks while local movement is pending so Zone-ready actions are not left waiting for another keyboard packet.
- Fixed shared Zone movement queue semantics: same-tick input bursts still coalesce to the latest intent, but a ready/near-ready queued Walk/Run is consumed before a later direction replaces the queue tail. This prevents silent ACK loss when held run hits a boundary and the next direction arrives.
- Updated the movement suite run order to avoid the starter east-field transfer gate at `x=339..341,y=268..271`; core movement acceptance now tests Run Left before Run Right so the right run returns toward spawn instead of entering the map-transfer flow.
- Verification: `node --check apps/web/scripts/run-probe.mjs`, Web `npx tsc --noEmit --pretty false`, `cargo +1.89.0 fmt --check -p mir2-simulation`, `cargo +1.89.0 test -p mir2-simulation --test shared_zone -- --nocapture` (146/146), and targeted `git diff --check` passed.
- Final local headless evidence: `docs/generated/player-qa/mir2-probe/mir2-probe-movement-suite-final-candidate-20260706025111.json` and `docs/generated/player-qa/mir2-probe/mir2-probe-movement-suite-final-candidate-20260706025111-movement-suite-summary.json` passed with 8/8 movement cases, 0 failures, 0 warnings, each walk/run direction at 6 sent / 6 ACK, ACK P95 33-41ms, final queue empty, final prediction cleared, render/server position match, and no action-window longtask/RAF/timer spikes.
- Final local headed Chromium evidence: `docs/generated/player-qa/mir2-probe/mir2-probe-movement-suite-headed-final-20260706031025.json` and `docs/generated/player-qa/mir2-probe/mir2-probe-movement-suite-headed-final-20260706031025-movement-suite-summary.json` also passed with 8/8 movement cases, 0 failures, 0 warnings, each direction at 6 sent / 6 ACK, ACK P95 35-43ms, steady cadence P95 601-606ms, and no action-window longtask/RAF/timer spikes.
- Local plain-URL login fix: the user's `http://127.0.0.1:13010` page could not enter the game because the running Next dev server had no `NEXT_PUBLIC_MIR2_GATEWAY_WS_URL` and therefore defaulted to `ws://127.0.0.1:7110/ws`; earlier probe runs hid this by passing `?gatewayWs=ws://127.0.0.1:7210/ws`. Restarted the 13010 Next dev server with `NEXT_PUBLIC_MIR2_GATEWAY_WS_URL=ws://127.0.0.1:7210/ws` while keeping the existing gateway on 7210. Evidence: `docs/generated/player-qa/mir2-probe/mir2-probe-plain-url-login-20260706090520.json` passed from a URL without `gatewayWs`, ending at `screen=game`, `wsState=open`, map `BichonProvince`, player `330:270`, and no critical console errors or non-favicon 404s. A second plain URL `demo/demo` check also passed in `docs/generated/player-qa/mir2-probe/mir2-probe-plain-url-demo-20260706091349.json`, ending at `screen=game`, `wsState=open`, map `BichonProvince`, character `Scout`, position `342:262`.

2026-08-06 cross-platform Bevy M0/M1 contract:

- Added a dependency-free `mir2-client-core` crate and moved snapshot interpolation and motion-window math behind thin Bevy adapters. Added deterministic Clock/ManualClock, intent envelope/sequencer and snapshot revision-gate primitives without defining concrete gameplay or wire commands.
- Added ADR-0001, the frozen M1 client-model contract and a bounded M1-A prompt for DeepSeek V4 Flash. Simulation, Gateway, protocol, React gameplay and platform-shell code remain outside the allowed next slice.
- Verification passed: client-core 12/12, Bevy runtime 19/19, dependency tree contains only client-core, WebGPU and WebGL2 dev/release WASM builds, Next.js production build with TypeScript/static generation, backend browser matrix all assertions green, and `git diff --check`.
- The required generic Playwright client was run and screenshots were inspected. The login surface rendered correctly; once a WebGPU canvas existed the generic helper captured that canvas rather than the DOM login surface. The raw WebGL2 QA route reported `renderedLayers=1` through the dedicated backend smoke, while generic canvas `toDataURL` captures remained black in both headless and headed mode. Treat this as a pre-existing capture/QA limitation, not real-device visual acceptance.
- Build-generated WASM bindings, binaries and runtime-version metadata were restored after verification and are not part of this source commit.
- Next task: Flash implements only M1-A deterministic motion-clock injection, runs every acceptance command in `docs/architecture/M1-CLIENT-MODEL-CONTRACT.md`, and leaves its diff uncommitted for Sol review.
