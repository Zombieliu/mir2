...
export function OriginalClientShell({
  ...
  // [DEBUG-mir2-render→fix] 图集 rect keys, 用于 DOM 实体层按实体回退:
  // Bevy 图集只覆盖 starter 实体, 图集外实体的帧在 Bevy 查不到 rect → 闪烁.
  // EntitySpriteLayers 用这个集合判断: 图集内 layer 走 Bevy, 图集外走 DOM(完整帧).
  const bevyEntityAtlasRectsForDomFallback = useMemo(() => {
    const rectList = activeBevyEntityAtlas?.rectList ?? [];
    if (rectList.length === 0) return null;
    return new Set(rectList.map((rect) => rect.key));
  }, [activeBevyEntityAtlas?.rectList]);
        ...
        >
          ...
          >
            ...
            <OriginalClientSceneVisualLayers
              ...
              bevyEntityAtlasRects={bevyEntityAtlasRectsForDomFallback}
...
function shouldUseBevyEntityAtlas() {
  ...
  // [DEBUG-mir2-render→fix] 默认启用 Bevy 动态实体图集. 之前的默认(false)让 Bevy 用
  // "单图 path 加载" 渲染实体: 真实 GPU(WebGPU/WebGL2)上每帧按 path 加载帧图, 图集外实体
  // 查不到 rect, 逐帧加载竞争 → 所有带动画的实体在真实浏览器上闪烁. 动态图集把所有当前
  // 场景实体帧打包进 GPU 图集, 动画只切换 rect(不重建, key 稳定), 完全消除闪烁.
  // 历史 2GiB 内存问题已由 BevyEntityAtlasBudgetProfile(160MB 内存/256MB 持久化预算)管理.
  // 逃生开关: ?bevyAtlas=0 或 localStorage mir2-bevy-atlas=0.
  return true;
}
