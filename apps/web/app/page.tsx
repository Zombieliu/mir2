...
export default function HomePage() {
  ...
  // [DEBUG-mir2-render→fix] 客户端实体滞留 (entity grace period):
  // 服务器 AOI 在玩家移动时会在边界反复加入/移除同一批实体, 若客户端立即镜像该集合,
  // 边界实体会反复挂载/卸载 → 闪烁。这里在实体从服务器集合消失后保留 `ENTITY_GRACE_MS`
  // 再移除, 稳定边界抖动; 超过宽限期的实体(真实离开/死亡)仍正常消失。
  const ENTITY_GRACE_MS = 2000;
  const entityGraceRef = useRef<Record<string, { entity: WorldEntity; lastSeen: number }>>({});
  const displayEntities = useMemo(() => {
    const now = Date.now();
    const grace = entityGraceRef.current;
    const merged = new Map<string, WorldEntity>();
    for (const entity of world.entities) {
      merged.set(String(entity.objectId), entity as WorldEntity);
      grace[String(entity.objectId)] = { entity: entity as WorldEntity, lastSeen: now };
    }
    // 保留仍在宽限期内的已消失实体, 避免 AOI 边界抖动造成闪烁
    for (const [id, record] of Object.entries(grace)) {
      if (merged.has(id)) continue;
      if (now - record.lastSeen < ENTITY_GRACE_MS) {
        merged.set(id, record.entity);
      } else {
        delete grace[id];
      }
    }
    const mergedEntities = Array.from(merged.values());
  ...
  // AOI visibility with a hysteresis band: entities only enter when inside the
  // strict range, but once visible they stay visible until they leave an outer
  // margin. Without this, monsters/NPCs that idle at the viewport edge drift in
  // and out of range every frame, unmounting/remounting their sprite layers and
  // visibly flickering. The margin keeps combat/social intent intact while
  // stabilising edge rendering.
  const viewportEntityEnteredRef = useRef(new Set<string>());
  const VIEWPORT_HYSTERESIS_MARGIN = 4;
  const viewportEntities = useMemo(() => {
    if (!viewportCenter) return [];
    const entered = viewportEntityEnteredRef.current;
    const nextEntered = new Set<string>();
    ...
    const visible = sortedEntities.filter((entity) => {
      const dx = Math.abs(entity.x - viewportCenter.x);
      const dy = Math.abs(entity.y - viewportCenter.y);
      const wasVisible = entered.has(entity.objectId);
      const inside = dx <= DEFAULT_VIEWPORT_LAYOUT.rangeX && dy <= DEFAULT_VIEWPORT_LAYOUT.rangeY;
      const withinMargin =
        dx <= DEFAULT_VIEWPORT_LAYOUT.rangeX + VIEWPORT_HYSTERESIS_MARGIN &&
        dy <= DEFAULT_VIEWPORT_LAYOUT.rangeY + VIEWPORT_HYSTERESIS_MARGIN;
      const keepVisible = inside || (wasVisible && withinMargin);
      if (keepVisible) {
        nextEntered.add(entity.objectId);
      }
      return keepVisible;
    });
    viewportEntityEnteredRef.current = nextEntered;
