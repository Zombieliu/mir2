...
function frameMetaForIndexWithFallback(
  ...
  // [DEBUG-mir2-render→fix] 帧缺失回退不再跳回第0帧(站立):
  // actor 库的 meta.json 只 ship 移动帧(80), 但 frame-set 要求攻击/受击/死亡等动作帧(80+).
  // 旧实现缺失帧直接回退到 frames[0], 动画在"动作帧↔站立帧"间来回跳 = 所有带动画的实体闪烁.
  // 现在缺失帧用模运算映射到有效帧, 动画平滑连续; fallback 帧优先(保持动作帧语义).
  const direct = frameMetaForIndex(library, frameIndex);
  if (direct) return direct;
  const fallback = fallbackFrameIndex === null ? null : frameMetaForIndex(library, fallbackFrameIndex);
  if (fallback) return fallback;
  if (library && library.frames.length > 0) {
    // 对缺失帧做模运算, 保证动画索引单调连续而不是跳回站立帧
    const wrappedIndex = ((frameIndex % library.frames.length) + library.frames.length) % library.frames.length;
    return library.frames[wrappedIndex] ?? library.frames[0] ?? null;
  }
  return null;
}
