...
type EntitySpriteLayersProps = {
  ...
  // [DEBUG-mir2-render→fix] Bevy 实体图集只覆盖 starter 实体(玩家装扮 + 4种怪物 + 部分NPC).
  // 图集外的实体(Monster/003/004/005 等)在 Bevy 里查不到 rect → 闪烁. 用图集 rect keys
  // 按实体判断: 图集覆盖的 layer 走 Bevy(不渲染 DOM img), 图集外的渲染 DOM img(帧完整, 不闪).
  bevyEntityAtlasRects?: ReadonlySet<string> | null;
};
...
const EntitySpriteLayers = memo(function EntitySpriteLayers({
  ...
  sprite,
  objectId,
  bevyEntityAtlasRects,
}: EntitySpriteLayersProps) {
  // [DEBUG-mir2-render→fix] 单个 layer 是否渲染 DOM <img>: Bevy 启用时,
  // 若该 layer 的帧在图集 rect 里则走 Bevy(隐藏 DOM img), 否则走 DOM img.
  // 图集外实体由此回退到完整帧的 DOM 渲染, 避免 Bevy 查不到 rect 而闪烁.
  const layerInAtlas = (path: string, width: number, height: number) => {
    if (!useBevyEntityRenderer || !bevyEntityAtlasRects) return false;
    const key = `${path}|${Math.max(1, Math.ceil(width))}x${Math.max(1, Math.ceil(height))}`;
    return bevyEntityAtlasRects.has(key);
  };
  // 该 layer 是否走 DOM <img>: Bevy 启用且帧在图集时走 Bevy, 否则走 DOM.
  const shouldRenderDom = (path: string, width: number, height: number) =>
    !useBevyEntityRenderer || !layerInAtlas(path, width, height);
  return (
    <>
      {shouldRenderDom(sprite?.mount?.path ?? "", sprite?.mount?.width ?? 0, sprite?.mount?.height ?? 0) && sprite?.mount ? (
        <img
      ...
      {sprite?.rearWeapons
        .filter((weapon) => shouldRenderDom(weapon.path, weapon.width, weapon.height))
        .map((weapon, index) => (
          <img
      ...
      {shouldRenderDom(sprite?.body?.path ?? "", sprite?.body?.width ?? 0, sprite?.body?.height ?? 0) && sprite?.body ? (
        <img
      ...
      {shouldRenderDom(sprite?.hair?.path ?? "", sprite?.hair?.width ?? 0, sprite?.hair?.height ?? 0) && sprite?.hair ? (
        <img
          ...
          return (
            ...
            >
              ...
              <EntitySpriteLayers
                ...
                sprite={sprite}
                objectId={entity.objectId}
                bevyEntityAtlasRects={bevyEntityAtlasRects}
              />
            </div>
          );
        })}
