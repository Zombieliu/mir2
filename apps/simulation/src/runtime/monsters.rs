...
pub(super) fn crystal_speed_to_ticks(speed_ms: u16) -> u64 {
    // [parity-fix] Crystal's MoveSpeed/AttackSpeed are millisecond intervals
    // (MonsterObject.cs: `MoveTime = Envir.Time + MoveSpeed`). The Rust runtime
    // advances one tick per 300 ms (see resources.rs CRYSTAL_PACKET_ACTION_TICK_MS
    // and zone/runtime.rs `ticks * 300`), NOT 1000 ms. The old div_ceil(1000)
    // forced every speed <= 1000 ms to 1 tick, so monsters moved 3+ times too
    // fast (a 700 ms step ran every 300 ms). Convert against the real 300 ms tick.
    u64::from(speed_ms.max(400)).div_ceil(300)
}
